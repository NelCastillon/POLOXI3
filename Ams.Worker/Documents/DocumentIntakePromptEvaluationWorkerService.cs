            ...
            try
            {
                ...
                foreach(var run in await repository.LeasePromptEvaluationsAsync(_leaseOwner,5,stoppingToken))
                {
                    ...
                    foreach(var test in run.Cases)
                    {
                        ...
                        try
                        {
                            var tenantId=run.TenantId??throw new InvalidOperationException("Prompt evaluation requires tenant scope.");var input=await payloads.ReadJsonAsync(tenantId,null,test.InputPayloadReference,stoppingToken);var output=await provider.InterpretAsync(new(tenantId,run.RunId,run.RunId,Guid.Empty,"PROMPT_EVALUATION",run.PromptCode,run.PromptVersion,run.SystemPrompt,run.OutputSchemaJson,input,run.CorrelationId),stoppingToken);var actual=await payloads.SaveJsonAsync(tenantId,run.RunId,"prompt-evaluation",output.OutputJson,stoppingToken);var score=Score(test.ExpectedOutputJson,output.OutputJson,test.EvaluationRulesJson);results.Add(new(test.CaseId,score>=0.999m?"PASSED":"FAILED",score,actual,score>=0.999m?null:BuildDifference(test.ExpectedOutputJson,output.OutputJson),null,null,clock.ElapsedMilliseconds));
                        }
