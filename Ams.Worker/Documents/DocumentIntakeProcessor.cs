...
{
    Task<DocumentIntakeBatchResult> ProcessBatchAsync(string leaseOwner,int batchSize,TimeSpan leaseDuration,CancellationToken cancellationToken=default);
}
...
public sealed class DocumentIntakeProcessor : IDocumentIntakeProcessor
{
    ...
    public DocumentIntakeProcessor(IDocumentIntakeRepository repository,IDocumentIntakeOperationsRepository operations,IDocumentStorageService storage,IDocumentIntakePayloadStore payloads,IDocumentOcrProvider ocr,IDocumentInterpretationProvider interpretation,IDocumentKnowledgeNormalizer normalizer,IDocumentSearchIndexer search,IRulesPlatformService rules,IValidationPlatformService validation){_repository=repository;_operations=operations;_storage=storage;_payloads=payloads;_ocr=ocr;_interpretation=interpretation;_normalizer=normalizer;_search=search;_rules=rules;_validation=validation;}

    public async Task<DocumentIntakeBatchResult> ProcessBatchAsync(string leaseOwner,int batchSize,TimeSpan leaseDuration,CancellationToken cancellationToken=default)
    {
        var completed=0;var retried=0;var failed=0;var items=await _repository.LeaseWorkItemsAsync(leaseOwner,Math.Clamp(batchSize,1,100),leaseDuration,cancellationToken);
