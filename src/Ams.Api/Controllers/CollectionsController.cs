...
[ApiController]
[Route("api/billing/collections")]
public sealed class CollectionsController : ControllerBase
{
    private readonly ICollectionsNoteService _service;
    public CollectionsController(ICollectionsNoteService service) => _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateCollectionsNoteRequest request, CancellationToken cancellationToken)
        => Ok(await _service.CreateAsync(request, cancellationToken));

    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateCollectionsNoteRequest request, CancellationToken cancellationToken)
    {
