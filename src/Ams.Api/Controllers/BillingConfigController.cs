...
public sealed class BillingConfigController : ControllerBase
{
    private readonly IBillingConfigService _service;
    public BillingConfigController(IBillingConfigService service) => _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateBillingConfigItemRequest request, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateBillingConfigItemRequest request, CancellationToken ct)
    {
