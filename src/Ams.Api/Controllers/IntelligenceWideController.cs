...
// api/intelligence/search/eph endpoint so the wide path can evolve independently.
[ApiController]
...
public sealed class IntelligenceWideController(IIntelligenceWideService service):ControllerBase
{
    ...
    [HttpPost("search/eph_wide")]
    [Authorize(Policy=IntelligencePolicies.Search)]
    public async Task<IActionResult> SearchWithEphWide([FromBody]EphSearchRequest request,CancellationToken cancellationToken)=>Ok(await service.SearchWithEphWideAsync(request with{TenantId=TenantId,UserId=ActorUserId,GrantedPermissions=AuthenticatedRequestContext.GetGrantedPermissions(User)},cancellationToken));
