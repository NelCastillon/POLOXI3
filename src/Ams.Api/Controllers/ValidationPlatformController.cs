...
[ApiController]
[Route("api/platform/validation")]
public sealed class ValidationPlatformController(IValidationPlatformService service) : ControllerBase
{
    [HttpPost("execute")]
    [Authorize(Policy = IntelligencePolicies.Evaluate)]
    public async Task<IActionResult> Execute([FromBody] ExecuteValidationsRequest request, CancellationToken cancellationToken)
    {
        ...
        return Ok(await service.ValidateAsync(request with { TenantId = tenantId, ActorUserId = actorUserId }, cancellationToken));
    }
}
