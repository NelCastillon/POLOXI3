...
[ApiController]
[Route("api/marketing-config")]
public sealed class MarketingConfigController : ControllerBase
{
    private readonly IMarketingConfigService _service;
    public MarketingConfigController(IMarketingConfigService service) => _service = service;
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateMarketingConfigItemRequest request, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateMarketingConfigItemRequest request, CancellationToken ct)
    {
