...
public sealed class AiConfigService : IAiConfigService
{
    ...
    public Task<AiConfigItemDto?> GetByIdAsync(Guid id, CancellationToken ct = default) => _repo.GetByIdAsync(id, ct);
    ...
    public Task UpdateAsync(Guid id, UpdateAiConfigItemRequest request, CancellationToken ct = default) => _repo.UpdateAsync(id, request, ct);
