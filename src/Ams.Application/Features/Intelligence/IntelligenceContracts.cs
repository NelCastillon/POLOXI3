...
public sealed record EphSearchRequest(Guid TenantId,Guid UserId,[Required,StringLength(1000,MinimumLength=2)]string Query,[Range(1,100)]int MaximumResults=25,[Required,StringLength(120)]string CorrelationId="")
{
    public bool IncludeExplanation { get; init; } = true;
    public bool UseEphEngine { get; init; } = true;
    public IReadOnlyCollection<string> GrantedPermissions { get; init; } = [];
}
public sealed record EphCapabilityDto(Guid CapabilityId,string CapabilityCode,string DisplayName,string Description,string EntityTypeCode,string ModuleCode,string ExecutionHandlerCode,IReadOnlyCollection<string> ApprovedTerms,bool SupportsRecency,decimal MinimumConfidence,int SortOrder);
public sealed record EphConfiguration(bool EnableHierarchyReuse,int HierarchyCacheHours,decimal MinimumBranchConfidence,int MaximumBranches,int MaximumResults);
public sealed record EphProposedBranch(string BranchCode,string DisplayName,string Condition,string? CapabilityCode,string? SearchText,bool OrderByRecency,decimal Confidence,IReadOnlyCollection<EphProposedBranch> Children);
public sealed record EphHierarchyProposal(string ConceptCode,string DisplayName,decimal Confidence,IReadOnlyCollection<EphProposedBranch> Branches);
public sealed record EphHierarchyRecord(Guid HierarchyId,string QuerySignature,string ConceptCode,string DisplayName,string NormalizedQuery,int VersionNumber,string StatusCode,string? GeneratedByProviderCode,string? GeneratedByModelCode,decimal Confidence,int UsageCount,int SuccessfulUsageCount,DateTime? ExpiresDateUtc,IReadOnlyCollection<EphBranchRecord> Branches);
public sealed record EphBranchRecord(Guid HierarchyBranchId,Guid? ParentHierarchyBranchId,string BranchCode,string DisplayName,string ProposedCondition,string? CapabilityCode,string ValidationStatusCode,string? ValidationMessage,string? SearchText,bool OrderByRecency,decimal Confidence,int SortOrder);
public sealed record EphEvidenceDto(Guid HierarchyBranchId,Guid SearchDocumentId,string EntityTypeCode,Guid EntityId,string ModuleCode,string Title,string? Excerpt,string? NavigationRoute,decimal RelevanceScore,int RankNumber,IReadOnlyCollection<string> MatchedBranches);
public sealed record EphSearchResponse(Guid EphExecutionId,Guid HierarchyId,string Query,string ConceptCode,string ConceptDisplayName,int HierarchyVersion,bool WasHierarchyReused,decimal Confidence,IReadOnlyCollection<EphBranchRecord> Branches,IReadOnlyCollection<EphEvidenceDto> Evidence,string? GroundedExplanation,string ExplanationStatusCode,long DurationMilliseconds);
public sealed record EphExecutionStart(Guid TenantId,Guid HierarchyId,Guid UserId,string QueryText,string CorrelationId,bool WasHierarchyReused,int ValidBranchCount,int UnsupportedBranchCount,decimal Confidence);
