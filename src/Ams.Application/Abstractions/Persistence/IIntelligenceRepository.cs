...
public interface IIntelligenceRepository
{
    ...
    Task<PoloxiConfiguration> GetPoloxiConfigurationAsync(Guid tenantId,CancellationToken cancellationToken=default);
    Task<IReadOnlyCollection<PoloxiCapabilityDto>> GetPoloxiCapabilitiesAsync(Guid tenantId,CancellationToken cancellationToken=default);
    Task<PoloxiHierarchyRecord?> GetReusablePoloxiHierarchyAsync(Guid tenantId,string querySignature,CancellationToken cancellationToken=default);
    Task<PoloxiHierarchyRecord> SavePoloxiHierarchyAsync(Guid tenantId,Guid userId,string querySignature,string normalizedQuery,PoloxiHierarchyProposal proposal,string? providerCode,string? modelCode,DateTime expiresDateUtc,IReadOnlyCollection<PoloxiBranchRecord> branches,CancellationToken cancellationToken=default);
    Task<IReadOnlyCollection<PoloxiEvidenceDto>> ExecutePoloxiBranchAsync(PoloxiSearchRequest request,PoloxiBranchRecord branch,PoloxiCapabilityDto capability,int maximumResults,CancellationToken cancellationToken=default);
    Task<Guid> StartPoloxiExecutionAsync(PoloxiExecutionStart execution,CancellationToken cancellationToken=default);
    Task CompletePoloxiExecutionAsync(Guid tenantId,Guid userId,Guid poloxiExecutionId,Guid hierarchyId,IReadOnlyCollection<PoloxiEvidenceDto> evidence,string explanationStatusCode,string? explanation,long durationMilliseconds,CancellationToken cancellationToken=default);
