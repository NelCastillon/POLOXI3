...
public sealed class DeferredRevenueRecognitionRepository : IDeferredRevenueRecognitionRepository
{
    ...
    private static readonly string _searchSql = RepositorySql.BuildPagedSearchSql(
        "Finance.DeferredRevenueRecognition",
        "RecognitionId, TenantId, DeferredRevenueScheduleId, RecognitionDate, Amount, JournalEntryId, StatusCode, CreatedDateUtc, CreatedByUserId, IsDeleted",
        "StatusCode LIKE '%' + @SearchTerm + '%'",
        "RecognitionDate DESC");
    ...
    {
        const string sql = "SELECT RecognitionId, TenantId, DeferredRevenueScheduleId, RecognitionDate, Amount, JournalEntryId, StatusCode, CreatedDateUtc, CreatedByUserId, IsDeleted FROM Finance.DeferredRevenueRecognition WHERE RecognitionId = @Id AND IsDeleted = 0;";
