...
public sealed class SubscriptionSettingsWorkflowRepository : ISubscriptionSettingsWorkflowRepository
{
    private readonly ISqlConnectionFactory _connectionFactory;

    public SubscriptionSettingsWorkflowRepository(ISqlConnectionFactory connectionFactory)
        => _connectionFactory = connectionFactory;

    public async Task<IReadOnlyList<SubscriptionSettingsWorkflowItemDto>> GetByPageAsync(Guid tenantId, string pageCode, CancellationToken cancellationToken = default)
    {
        const string sql = """
            ...
            FROM Core.SubscriptionSettingsWorkflowItem
        ...
        var items = await cn.QueryAsync<SubscriptionSettingsWorkflowItemDto>(
            new CommandDefinition(sql, new { TenantId = tenantId, PageCode = pageCode }, cancellationToken: cancellationToken));
        return items.AsList();
    }

    public async Task<Guid> CreateAsync(CreateSubscriptionSettingsWorkflowItemRequest request, CancellationToken cancellationToken = default)
    {
        var id = Guid.NewGuid();
        const string sql = """
    ...
    public async Task UpdateAsync(Guid workflowItemId, UpdateSubscriptionSettingsWorkflowItemRequest request, CancellationToken cancellationToken = default)
    {
        const string sql = """
            UPDATE Core.SubscriptionSettingsWorkflowItem
            SET Title = @Title,
    ...
    public async Task AdvanceAsync(Guid workflowItemId, AdvanceSubscriptionSettingsWorkflowRequest request, CancellationToken cancellationToken = default)
    {
        const string sql = """
            UPDATE Core.SubscriptionSettingsWorkflowItem
