...
""";
    ...
    public async Task<InsuranceReasoningResponse> ExecuteReasoningAsync(InsuranceReasoningRequest request,IReadOnlyCollection<SemanticConceptMatchDto> concepts,CancellationToken cancellationToken=default)
    {
        ...
        const string sql="""
            ...
            INSERT AI.ReasoningEvidence(ReasoningEvidenceId,TenantId,ReasoningSessionId,EvidenceTypeCode,SourceModuleCode,SourceEntityTypeCode,SourceEntityId,SourceReference,Title,Summary,EvidenceValueJson,RelevanceScore,IsAuthoritative,CreatedDateUtc,CreatedByUserId,IsDeleted)
            SELECT NEWID(),@TenantId,@SessionId,N'CARRIER_RULE',N'Agency',N'CARRIER_PRODUCT_RULE',rule.CarrierProductRuleId,N'Agency.CarrierProductRule:'+CONVERT(nvarchar(36),rule.CarrierProductRuleId),rule.RuleName,COALESCE(rule.RuleDescription,N'Active carrier product rule applies to this bind transaction.'),CONCAT(N'{"ruleCode":"',STRING_ESCAPE(rule.RuleCode,'json'),N'","carrier":"',STRING_ESCAPE(COALESCE(rule.CarrierName,N''),'json'),N'","state":"',STRING_ESCAPE(COALESCE(rule.StateCode,N''),'json'),N'","requireSignedApplication":',CASE WHEN rule.RequireSignedApplication=1 THEN N'true' ELSE N'false' END,N',"requireLossRuns":',CASE WHEN rule.RequireLossRuns=1 THEN N'true' ELSE N'false' END,N',"requireUnderwriterApproval":',CASE WHEN rule.RequireUnderwriterApproval=1 THEN N'true' ELSE N'false' END,N'}'),.9500,1,SYSUTCDATETIME(),@UserId,0
            FROM Submissions.PolicyBindTransaction bind JOIN Submissions.Submission submission ON submission.TenantId=bind.TenantId AND submission.SubmissionId=bind.SubmissionId AND submission.IsDeleted=0 JOIN Agency.CarrierProductRule rule ON rule.TenantId=bind.TenantId AND rule.IsDeleted=0 AND rule.IsActive=1 AND (rule.CarrierId IS NULL OR rule.CarrierId=bind.CarrierId) AND (rule.LineOfBusinessCode IS NULL OR rule.LineOfBusinessCode=submission.LineOfBusiness) AND rule.EffectiveDate<=CONVERT(date,SYSUTCDATETIME()) AND (rule.ExpirationDate IS NULL OR rule.ExpirationDate>=CONVERT(date,SYSUTCDATETIME()))
