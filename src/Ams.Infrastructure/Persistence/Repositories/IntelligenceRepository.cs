...
public sealed partial class IntelligenceRepository(ISqlConnectionFactory connectionFactory):IIntelligenceRepository,IRecommendationGenerationRepository
{
    ...
    {
        const string sql="""IF @RowVersion IS NOT NULL BEGIN UPDATE AI.Provider SET DisplayName=@DisplayName,ProviderTypeCode=@ProviderTypeCode,SupportsChat=@SupportsChat,SupportsEmbeddings=@SupportsEmbeddings,SupportsVision=@SupportsVision,SupportsStructuredOutput=@SupportsStructuredOutput,Priority=@Priority,IsActive=@IsActive,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE ProviderCode=@ProviderCode AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0 AND RowVersion=@RowVersion; IF @@ROWCOUNT=0 THROW 51000,'AI provider changed before this update.',1; END ELSE BEGIN INSERT AI.Provider(ProviderId,TenantId,ProviderCode,DisplayName,ProviderTypeCode,SupportsChat,SupportsEmbeddings,SupportsVision,SupportsStructuredOutput,Priority,IsActive,CreatedDateUtc,CreatedByUserId,IsDeleted) SELECT NEWID(),@TenantId,@ProviderCode,@DisplayName,@ProviderTypeCode,@SupportsChat,@SupportsEmbeddings,@SupportsVision,@SupportsStructuredOutput,@Priority,@IsActive,SYSUTCDATETIME(),@ActorUserId,0 WHERE NOT EXISTS(SELECT 1 FROM AI.Provider WITH(UPDLOCK,HOLDLOCK) WHERE ProviderCode=@ProviderCode AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0); IF @@ROWCOUNT=0 THROW 51000,'AI provider already exists; reload it before updating.',1; END;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);await connection.ExecuteAsync(new CommandDefinition(sql,request,cancellationToken:cancellationToken));
    }
    ...
    {
        const string sql="""IF NOT EXISTS(SELECT 1 FROM AI.Provider WHERE ProviderId=@ProviderId AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0) THROW 51000,'AI provider was not found.',1; IF @RowVersion IS NOT NULL BEGIN UPDATE AI.ModelDeployment SET ProviderId=@ProviderId,DeploymentName=@DeploymentName,ModelFamily=@ModelFamily,CapabilityCode=@CapabilityCode,ContextWindowTokens=@ContextWindowTokens,MaximumOutputTokens=@MaximumOutputTokens,InputCostPerMillionTokens=@InputCostPerMillionTokens,OutputCostPerMillionTokens=@OutputCostPerMillionTokens,CurrencyCode=@CurrencyCode,Priority=@Priority,IsFallback=@IsFallback,IsActive=@IsActive,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE ModelCode=@ModelCode AND ProviderId=@ProviderId AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0 AND RowVersion=@RowVersion; IF @@ROWCOUNT=0 THROW 51000,'AI model deployment changed before this update.',1; END ELSE BEGIN INSERT AI.ModelDeployment(ModelDeploymentId,TenantId,ProviderId,ModelCode,DeploymentName,ModelFamily,CapabilityCode,ContextWindowTokens,MaximumOutputTokens,InputCostPerMillionTokens,OutputCostPerMillionTokens,CurrencyCode,Priority,IsFallback,IsActive,CreatedDateUtc,CreatedByUserId,IsDeleted) SELECT NEWID(),@TenantId,@ProviderId,@ModelCode,@DeploymentName,@ModelFamily,@CapabilityCode,@ContextWindowTokens,@MaximumOutputTokens,@InputCostPerMillionTokens,@OutputCostPerMillionTokens,@CurrencyCode,@Priority,@IsFallback,@IsActive,SYSUTCDATETIME(),@ActorUserId,0 WHERE NOT EXISTS(SELECT 1 FROM AI.ModelDeployment WITH(UPDLOCK,HOLDLOCK) WHERE ModelCode=@ModelCode AND ProviderId=@ProviderId AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0); IF @@ROWCOUNT=0 THROW 51000,'AI model deployment already exists; reload it before updating.',1; END;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);await connection.ExecuteAsync(new CommandDefinition(sql,request,cancellationToken:cancellationToken));
    }
    ...
    {
        const string sql="""UPDATE AI.FeaturePolicy SET IsDeleted=1,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE TenantId=@TenantId AND FeatureCode=@FeatureCode AND IsDeleted=0; IF @@ROWCOUNT=0 THROW 51000,'Feature policy was not found.',1;""";
