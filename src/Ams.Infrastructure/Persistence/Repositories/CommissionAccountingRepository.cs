...
""";
    ...
    public async Task<CommissionMatchRunResultDto> SaveSharedMatchesAsync(RunCommissionMatchingRequest request, IReadOnlyCollection<ProposedCommissionMatch> matches, CancellationToken cancellationToken = default)
    {
        const string sql = """
CREATE TABLE #Matches(LineId UNIQUEIDENTIFIER,ExpectedId UNIQUEIDENTIFIER,Score DECIMAL(9,4),Method NVARCHAR(50),Received DECIMAL(18,2),Expected DECIMAL(18,2));
INSERT #Matches SELECT LineId,ExpectedId,Score,Method,Received,Expected FROM OPENJSON(@MatchesJson) WITH(LineId UNIQUEIDENTIFIER '$.carrierCommissionStatementLineId',ExpectedId UNIQUEIDENTIFIER '$.commissionExpectedReceivableId',Score DECIMAL(9,4) '$.matchScore',Method NVARCHAR(50) '$.matchMethodCode',Received DECIMAL(18,2) '$.matchedAmount',Expected DECIMAL(18,2) '$.expectedAmount');
...
SELECT NEWID(),@TenantId,match.LineId,match.ExpectedId,CONCAT(N'SearchMatching:',match.Method),match.Score,match.Received,match.Received-match.Expected,N'Proposed',SYSUTCDATETIME(),@UserId,SYSUTCDATETIME(),0 FROM #Matches match WHERE NOT EXISTS(SELECT 1 FROM Commission.CommissionReconciliationMatch existing WHERE existing.TenantId=@TenantId AND existing.CarrierCommissionStatementLineId=match.LineId AND existing.IsDeleted=0);
UPDATE line SET MatchStatusCode=N'Matched' FROM Commission.CarrierCommissionStatementLine line JOIN #Matches match ON match.LineId=line.CarrierCommissionStatementLineId WHERE line.TenantId=@TenantId;
...
SELECT NEWID(),@TenantId,@StatementId,line.CarrierCommissionStatementLineId,NULL,CONCAT(N'CRE-',FORMAT(SYSUTCDATETIME(),N'yyyyMMdd'),N'-',RIGHT(CONCAT(N'000000',ROW_NUMBER() OVER(ORDER BY line.LineNumber)),6)),N'UnmatchedStatementLine',N'High',N'Open',NULL,line.NetAmount,NULL,CONCAT(N'No configured Search & Matching candidate was approved for policy ',COALESCE(line.PolicyNumber,N'(missing)'),N'.'),SYSUTCDATETIME(),@UserId,0 FROM Commission.CarrierCommissionStatementLine line WHERE line.TenantId=@TenantId AND line.CarrierCommissionStatementId=@StatementId AND line.IsDeleted=0 AND line.MatchStatusCode=N'Unmatched' AND NOT EXISTS(SELECT 1 FROM Commission.CommissionReconciliationException existing WHERE existing.TenantId=@TenantId AND existing.CarrierCommissionStatementLineId=line.CarrierCommissionStatementLineId AND existing.StatusCode=N'Open' AND existing.IsDeleted=0);
UPDATE Commission.CarrierCommissionStatement SET ReconciliationStatusCode=CASE WHEN EXISTS(SELECT 1 FROM Commission.CarrierCommissionStatementLine WHERE CarrierCommissionStatementId=@StatementId AND IsDeleted=0 AND MatchStatusCode=N'Unmatched') THEN N'InProgress' ELSE N'Matched' END,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@UserId WHERE TenantId=@TenantId AND CarrierCommissionStatementId=@StatementId;
SELECT SUM(CASE WHEN Score>=98 THEN 1 ELSE 0 END) ExactMatches,SUM(CASE WHEN Score<98 THEN 1 ELSE 0 END) ToleranceMatches,(SELECT COUNT(1) FROM Commission.CarrierCommissionStatementLine WHERE TenantId=@TenantId AND CarrierCommissionStatementId=@StatementId AND IsDeleted=0 AND MatchStatusCode=N'Unmatched') UnmatchedLines,(SELECT COUNT(1) FROM Commission.CommissionReconciliationException WHERE TenantId=@TenantId AND CarrierCommissionStatementId=@StatementId AND IsDeleted=0 AND StatusCode=N'Open') ExceptionsCreated FROM #Matches;
""";
        using var connection = await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
        var result = await connection.QuerySingleAsync<MatchCounts>(new CommandDefinition(sql, new { request.TenantId, StatementId = request.CarrierCommissionStatementId, request.UserId, MatchesJson = JsonSerializer.Serialize(matches) }, cancellationToken: cancellationToken));
