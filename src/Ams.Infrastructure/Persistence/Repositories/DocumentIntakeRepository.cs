...
IF NOT EXISTS(SELECT 1 FROM DMS.IntakeSessionDocument WHERE TenantId=@TenantId AND IntakeSessionId=@IntakeSessionId) THROW 51000,'At least one evidence document is required.',1;";
    ...
    public Task ReviewFieldAsync(ReviewDocumentIntakeFieldCommand command, CancellationToken cancellationToken = default)
        ...
        {
            const string sql=@"
DECLARE @Previous NVARCHAR(MAX),@Extracted NVARCHAR(MAX); SELECT @Previous=ReviewedValue,@Extracted=ExtractedValue FROM DMS.IntakeDraftField WITH(UPDLOCK) WHERE TenantId=@TenantId AND IntakeSessionId=@IntakeSessionId AND IntakeDraftFieldId=@IntakeDraftFieldId AND RowVersion=@RowVersion;
IF @@ROWCOUNT=0 THROW 51000,'Draft field changed or was not found for tenant.',1;
DECLARE @New NVARCHAR(MAX)=CASE WHEN @DecisionCode=N'CORRECTED' THEN @ReviewedValue WHEN @DecisionCode=N'APPROVED' THEN COALESCE(@Previous,@Extracted) ELSE @Previous END;
...
UPDATE DMS.IntakeSession SET StatusCode=CASE WHEN NOT EXISTS(SELECT 1 FROM DMS.IntakeDraftField WHERE TenantId=@TenantId AND IntakeSessionId=@IntakeSessionId AND ReviewStatusCode=N'PENDING') AND NOT EXISTS(SELECT 1 FROM DMS.IntakeIssue WHERE TenantId=@TenantId AND IntakeSessionId=@IntakeSessionId AND SeverityCode=N'ERROR' AND StatusCode=N'OPEN') THEN N'READY' ELSE N'REVIEW_REQUIRED' END,WarningCount=(SELECT COUNT(1) FROM DMS.IntakeIssue WHERE TenantId=@TenantId AND IntakeSessionId=@IntakeSessionId AND SeverityCode=N'WARNING' AND StatusCode=N'OPEN'),ErrorCount=(SELECT COUNT(1) FROM DMS.IntakeIssue WHERE TenantId=@TenantId AND IntakeSessionId=@IntakeSessionId AND SeverityCode=N'ERROR' AND StatusCode=N'OPEN'),ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ReviewerUserId WHERE TenantId=@TenantId AND IntakeSessionId=@IntakeSessionId AND StatusCode IN(N'REVIEW_REQUIRED',N'READY');";
    ...
    public async Task ReprocessAsync(ReprocessDocumentIntakeCommand command, CancellationToken cancellationToken = default)
    {
        const string sql=@"
UPDATE DMS.IntakeSession SET StatusCode=N'QUEUED',ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE TenantId=@TenantId AND IntakeSessionId=@IntakeSessionId AND RowVersion=@RowVersion AND StatusCode IN(N'FAILED',N'REVIEW_REQUIRED',N'READY'); IF @@ROWCOUNT=0 THROW 51000,'Session changed or cannot be reprocessed.',1;
UPDATE DMS.IntakeWorkItem SET StatusCode=N'RETRY_SCHEDULED',AvailableDateUtc=SYSUTCDATETIME(),LeaseOwner=NULL,LeaseExpiresDateUtc=NULL,LastErrorCode=NULL,LastErrorMessage=NULL WHERE TenantId=@TenantId AND IntakeSessionId=@IntakeSessionId AND SequenceNumber>=(SELECT MIN(SequenceNumber) FROM DMS.IntakeWorkItem WHERE TenantId=@TenantId AND IntakeSessionId=@IntakeSessionId AND WorkTypeCode=@FromWorkTypeCode);";
        using var connection=await _connectionFactory.CreateOpenConnectionAsync(cancellationToken);await connection.ExecuteAsync(new CommandDefinition(sql,command,cancellationToken:cancellationToken));
    }
...
UPDATE DMS.IntakeWorkItem SET StatusCode=N'COMPLETED',CompletedDateUtc=SYSUTCDATETIME(),LeaseOwner=NULL,LeaseExpiresDateUtc=NULL WHERE IntakeWorkItemId=@Id AND LeaseOwner=@Owner;
