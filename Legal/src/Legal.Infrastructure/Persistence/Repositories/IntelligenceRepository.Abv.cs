...
// owner mappings, action catalog) and ABV resolution observability (POLOXI.Legal_Abv* tables, migration
...
FROM POLOXI.Legal_AbvDomainPack
...
SELECT IntentCode,Name,Description FROM POLOXI.Legal_AbvIntentTaxonomy
...
SELECT PolicyCode,IntentCode,ImpactTierCode,PriorityCode,SlaHours FROM POLOXI.Legal_AbvUrgencyPolicy
WHERE IsActive=1 AND IsDeleted=0 AND AbvDomainPackId=@PackId ORDER BY SortOrder;
""",new{PackId=pack.AbvDomainPackId},cancellationToken:cancellationToken)))
...
SELECT IntentCode,ImpactTierCode,OwnerRole FROM POLOXI.Legal_AbvOwnerMapping
WHERE IsActive=1 AND IsDeleted=0 AND AbvDomainPackId=@PackId ORDER BY SortOrder;
""",new{PackId=pack.AbvDomainPackId},cancellationToken:cancellationToken)))
...
SELECT IntentCode,ActionCode,Name,NextStep,PlaybookCode,ExecutionAllowed,HumanApprovalRequired FROM POLOXI.Legal_AbvActionCatalog
WHERE IsActive=1 AND IsDeleted=0 AND AbvDomainPackId=@PackId ORDER BY SortOrder;
""",new{PackId=pack.AbvDomainPackId},cancellationToken:cancellationToken)))
    ...
    {
        const string sql="""
INSERT POLOXI.Legal_AbvResolution
