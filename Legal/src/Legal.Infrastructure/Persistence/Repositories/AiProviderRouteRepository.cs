...
{
    public async Task<IReadOnlyCollection<AiProviderRoute>> GetRoutesAsync(Guid tenantId,string featureCode,string capabilityCode,string? modelCodeOverride=null,CancellationToken cancellationToken=default)
    {
        var effectiveModelCodeOverride=string.IsNullOrWhiteSpace(modelCodeOverride)||modelCodeOverride.Trim().Equals("Auto",StringComparison.OrdinalIgnoreCase)?"gpt-4.1-mini":modelCodeOverride.Trim();
        const string sql="""
WITH routes AS
(
    SELECT policy.TenantId,policy.FeatureCode,provider.ProviderCode,provider.ProviderTypeCode,model.ModelCode,model.DeploymentName,
           ...
           policy.TimeoutSeconds,policy.Temperature,policy.MaximumOutputTokens,-1 Priority,CONVERT(bit,0) IsFallback
    FROM AI.Legal_FeaturePolicy policy
    JOIN AI.Legal_ModelDeployment model ON model.ModelCode=@ModelCodeOverride AND model.CapabilityCode=@CapabilityCode AND model.IsActive=1 AND model.IsDeleted=0 AND (model.TenantId=policy.TenantId OR model.TenantId IS NULL)
    ...
    WHERE @ModelCodeOverride IS NOT NULL AND policy.TenantId=@TenantId AND policy.FeatureCode=@FeatureCode AND policy.IsEnabled=1 AND policy.IsDeleted=0
    UNION ALL
    SELECT policy.TenantId,policy.FeatureCode,provider.ProviderCode,provider.ProviderTypeCode,model.ModelCode,model.DeploymentName,
           endpointSetting.SettingValue EndpointReference,credentialSetting.SettingValue CredentialReference,apiVersion.SettingValue ApiVersion,
           policy.TimeoutSeconds,policy.Temperature,policy.MaximumOutputTokens,0 Priority,CONVERT(bit,0) IsFallback
    FROM AI.Legal_FeaturePolicy policy
    JOIN AI.Legal_ModelDeployment model ON model.ModelDeploymentId=policy.PrimaryModelDeploymentId AND model.IsActive=1 AND model.IsDeleted=0
    JOIN AI.Legal_Provider provider ON provider.ProviderId=model.ProviderId AND provider.IsActive=1 AND provider.IsDeleted=0
    OUTER APPLY(SELECT TOP(1) COALESCE(tenant.SettingValue,platform.SettingValue,platform.DefaultValue) SettingValue FROM Core.ConfigurationSetting platform LEFT JOIN Core.ConfigurationSetting tenant ON tenant.TenantId=policy.TenantId AND tenant.SettingKey=platform.SettingKey AND tenant.IsDeleted=0 WHERE platform.TenantId IS NULL AND platform.SettingKey=provider.EndpointConfigurationKey AND platform.IsDeleted=0) endpointSetting
    OUTER APPLY(SELECT TOP(1) COALESCE(tenant.SettingValue,platform.SettingValue,platform.DefaultValue) SettingValue FROM Core.ConfigurationSetting platform LEFT JOIN Core.ConfigurationSetting tenant ON tenant.TenantId=policy.TenantId AND tenant.SettingKey=platform.SettingKey AND tenant.IsDeleted=0 WHERE platform.TenantId IS NULL AND platform.SettingKey=provider.CredentialConfigurationKey AND platform.IsDeleted=0) credentialSetting
    OUTER APPLY(SELECT TOP(1) COALESCE(tenant.SettingValue,platform.SettingValue,platform.DefaultValue) SettingValue FROM Core.ConfigurationSetting platform LEFT JOIN Core.ConfigurationSetting tenant ON tenant.TenantId=policy.TenantId AND tenant.SettingKey=platform.SettingKey AND tenant.IsDeleted=0 WHERE platform.TenantId IS NULL AND platform.SettingKey=N'Intelligence.AzureOpenAi.ApiVersion' AND platform.IsDeleted=0) apiVersion
    WHERE @ModelCodeOverride IS NULL AND policy.TenantId=@TenantId AND policy.FeatureCode=@FeatureCode AND policy.IsEnabled=1 AND policy.IsDeleted=0 AND model.CapabilityCode=@CapabilityCode
    UNION ALL
    SELECT policy.TenantId,policy.FeatureCode,provider.ProviderCode,provider.ProviderTypeCode,model.ModelCode,model.DeploymentName,
    ...
    FROM AI.Legal_FeaturePolicy policy
    JOIN AI.Legal_ModelDeployment model ON model.ModelDeploymentId=policy.FallbackModelDeploymentId AND model.IsActive=1 AND model.IsDeleted=0
    JOIN AI.Legal_Provider provider ON provider.ProviderId=model.ProviderId AND provider.IsActive=1 AND provider.IsDeleted=0
    OUTER APPLY(SELECT TOP(1) COALESCE(tenant.SettingValue,platform.SettingValue,platform.DefaultValue) SettingValue FROM Core.ConfigurationSetting platform LEFT JOIN Core.ConfigurationSetting tenant ON tenant.TenantId=policy.TenantId AND tenant.SettingKey=platform.SettingKey AND tenant.IsDeleted=0 WHERE platform.TenantId IS NULL AND platform.SettingKey=provider.EndpointConfigurationKey AND platform.IsDeleted=0) endpointSetting
    OUTER APPLY(SELECT TOP(1) COALESCE(tenant.SettingValue,platform.SettingValue,platform.DefaultValue) SettingValue FROM Core.ConfigurationSetting platform LEFT JOIN Core.ConfigurationSetting tenant ON tenant.TenantId=policy.TenantId AND tenant.SettingKey=platform.SettingKey AND tenant.IsDeleted=0 WHERE platform.TenantId IS NULL AND platform.SettingKey=provider.CredentialConfigurationKey AND platform.IsDeleted=0) credentialSetting
    OUTER APPLY(SELECT TOP(1) COALESCE(tenant.SettingValue,platform.SettingValue,platform.DefaultValue) SettingValue FROM Core.ConfigurationSetting platform LEFT JOIN Core.ConfigurationSetting tenant ON tenant.TenantId=policy.TenantId AND tenant.SettingKey=platform.SettingKey AND tenant.IsDeleted=0 WHERE platform.TenantId IS NULL AND platform.SettingKey=N'Intelligence.AzureOpenAi.ApiVersion' AND platform.IsDeleted=0) apiVersion
...
""";
        ...
        return (await connection.QueryAsync<AiProviderRoute>(new CommandDefinition(sql,new{TenantId=tenantId,FeatureCode=featureCode,CapabilityCode=capabilityCode,ModelCodeOverride=effectiveModelCodeOverride},cancellationToken:cancellationToken))).AsList();
    }
...
(
    ...
    FROM AI.Legal_SafetyControl control WHERE control.IsActive=1 AND control.IsDeleted=0 AND (control.TenantId=@TenantId OR control.TenantId IS NULL)
)
...
""";
    ...
    {
        const string sql="""INSERT AI.Legal_SafetyEvent(TenantId,SafetyControlId,EventTypeCode,EnforcementStageCode,ActionCode,SeverityCode,InputHash,DetailsJson,RequiresHumanReview,ReviewStatusCode,DetectedDateUtc,CreatedDateUtc,IsDeleted,IdempotencyKey) SELECT @TenantId,@SafetyControlId,@EventTypeCode,@EnforcementStageCode,@ActionCode,@SeverityCode,@InputHash,@DetailsJson,@RequiresHumanReview,@ReviewStatusCode,SYSUTCDATETIME(),SYSUTCDATETIME(),0,@IdempotencyKey WHERE NOT EXISTS(SELECT 1 FROM AI.Legal_SafetyEvent WITH(UPDLOCK,HOLDLOCK) WHERE TenantId=@TenantId AND IdempotencyKey=@IdempotencyKey AND IsDeleted=0);""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
    ...
    {
        const string sql="""
INSERT AI.Legal_Execution(ExecutionId,TenantId,FeatureCode,ModuleCode,EntityTypeCode,EntityId,ProviderId,ModelDeploymentId,StatusCode,CorrelationId,StartedDateUtc,CompletedDateUtc,DurationMilliseconds,InputTokenCount,OutputTokenCount,Confidence,GroundingSourceCount,InputReference,ErrorCode,ErrorMessage,CreatedDateUtc,IsDeleted)
...
LEFT JOIN AI.Legal_Provider provider ON provider.ProviderCode=@ProviderCode AND provider.IsDeleted=0 AND (provider.TenantId=@TenantId OR provider.TenantId IS NULL)
LEFT JOIN AI.Legal_ModelDeployment model ON model.ProviderId=provider.ProviderId AND model.ModelCode=@ModelCode AND model.IsDeleted=0 AND (model.TenantId=@TenantId OR model.TenantId IS NULL)
WHERE NOT EXISTS(SELECT 1 FROM AI.Legal_Execution WITH(UPDLOCK,HOLDLOCK) WHERE TenantId=@TenantId AND CorrelationId=@CorrelationId AND FeatureCode=@FeatureCode AND IsDeleted=0);
IF @@ROWCOUNT>0 AND @GroundingSourceReference IS NOT NULL
INSERT AI.Legal_ExecutionGroundingSource(ExecutionGroundingSourceId,TenantId,ExecutionId,SourceTypeCode,SourceEntityId,SourceReference,Title,CreatedDateUtc,IsDeleted)
