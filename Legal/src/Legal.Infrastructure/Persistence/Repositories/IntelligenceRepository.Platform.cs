...
public sealed partial class IntelligenceRepository
{
    ...
    {
        const string sql="""
            SELECT IntelligencePillarId,PillarCode,DisplayName,Description,SortOrder,IsActive FROM AI.Legal_IntelligencePillar WHERE (TenantId IS NULL OR TenantId=@TenantId) AND IsDeleted=0 ORDER BY SortOrder;
            SELECT IntelligenceCapabilityId,IntelligencePillarId,CapabilityCode,DisplayName,Description,EngineKindCode,OwningModuleCode,IsAdvisory,RequiresHumanReview,SortOrder,IsActive FROM AI.Legal_IntelligenceCapability WHERE (TenantId IS NULL OR TenantId=@TenantId) AND IsDeleted=0 ORDER BY IntelligencePillarId,SortOrder;
            SELECT
              (SELECT COUNT(1) FROM AI.Legal_IntelligenceFinding WHERE TenantId=@TenantId AND IsDeleted=0 AND StatusCode IN(N'OPEN',N'REVIEW_REQUIRED',N'IN_REVIEW')) OpenFindingCount,
              (SELECT COUNT(1) FROM AI.Legal_BusinessSignal WHERE TenantId=@TenantId AND IsDeleted=0 AND StatusCode IN(N'OPEN',N'REVIEW_REQUIRED',N'IN_PROGRESS')) OpenBusinessSignalCount,
              (SELECT COUNT(1) FROM AI.Legal_ReasoningSession WHERE TenantId=@TenantId AND IsDeleted=0 AND StatusCode IN(N'QUEUED',N'PROCESSING')) ActiveReasoningSessionCount,
              (SELECT COUNT(1) FROM AI.Legal_IntelligenceWorkItem WHERE TenantId=@TenantId AND IsDeleted=0 AND StatusCode IN(N'PENDING',N'PROCESSING',N'RETRY')) PendingWorkItemCount;
            """;
...
""";
    ...
    {
        const string sql="""SELECT ComplianceRequirementId,RequirementCode,DisplayName,Description,RequirementScopeCode,JurisdictionCode,CarrierId,LineOfBusinessCode,EntityTypeCode,RequirementTypeCode,SeverityCode,BlocksTransaction,CanBeWaived,WaiverPermissionCode,ApprovalPermissionCode,VersionNumber,IsActive,RowVersion FROM AI.Legal_ComplianceRequirement WHERE TenantId=@TenantId AND IsDeleted=0 ORDER BY RequirementScopeCode,DisplayName,VersionNumber DESC;""";
    ...
    {
        const string sql="""
            IF NOT EXISTS(SELECT 1 FROM AI.Legal_IntelligenceCapability WHERE IntelligenceCapabilityId=@IntelligenceCapabilityId AND (TenantId IS NULL OR TenantId=@TenantId) AND IsDeleted=0) THROW 51000,'Intelligence capability was not found.',1;
            IF @StatusCode NOT IN(N'DRAFT',N'APPROVED',N'RETIRED') THROW 51000,'Prompt status is invalid.',1;
            IF @RowVersion IS NOT NULL
            BEGIN
              UPDATE AI.Legal_PromptDefinition SET IntelligenceCapabilityId=@IntelligenceCapabilityId,DisplayName=@DisplayName,SystemInstructions=@SystemInstructions,InputSchemaJson=@InputSchemaJson,OutputSchemaJson=@OutputSchemaJson,StatusCode=@StatusCode,ApprovedByUserId=CASE WHEN @StatusCode=N'APPROVED' THEN @ActorUserId ELSE NULL END,ApprovedDateUtc=CASE WHEN @StatusCode=N'APPROVED' THEN SYSUTCDATETIME() ELSE NULL END,EffectiveFromUtc=@EffectiveFromUtc,EffectiveToUtc=@EffectiveToUtc,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE (TenantId=@TenantId OR TenantId IS NULL) AND PromptCode=@PromptCode AND VersionLabel=@VersionLabel AND RowVersion=@RowVersion AND IsDeleted=0;
              IF @@ROWCOUNT=0 THROW 51000,'Prompt definition changed before this update.',1;
            END
            ELSE
            BEGIN
              INSERT AI.Legal_PromptDefinition(TenantId,IntelligenceCapabilityId,PromptCode,VersionLabel,DisplayName,SystemInstructions,InputSchemaJson,OutputSchemaJson,StatusCode,ApprovedByUserId,ApprovedDateUtc,EffectiveFromUtc,EffectiveToUtc,CreatedDateUtc,CreatedByUserId,IsDeleted)
              ...
              WHERE NOT EXISTS(SELECT 1 FROM AI.Legal_PromptDefinition WITH(UPDLOCK,HOLDLOCK) WHERE TenantId=@TenantId AND PromptCode=@PromptCode AND VersionLabel=@VersionLabel AND IsDeleted=0);
    ...
    {
        const string sql="""UPDATE AI.Legal_PromptDefinition SET IsDeleted=1,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE (TenantId=@TenantId OR TenantId IS NULL) AND PromptCode=@PromptCode AND VersionLabel=@VersionLabel AND IsDeleted=0; IF @@ROWCOUNT=0 THROW 51000,'Prompt version was not found.',1;""";
    ...
    {
        const string sql="""SELECT policy.EnginePolicyId,policy.TenantId,policy.IntelligenceCapabilityId,capability.CapabilityCode,policy.PolicyCode,policy.DisplayName,policy.Description,policy.ExecutionModeCode,policy.ConfigurationJson,policy.MinimumConfidence,policy.RequiresHumanReview,policy.FailClosed,policy.EffectiveFromUtc,policy.EffectiveToUtc,policy.VersionNumber,policy.IsActive,policy.RowVersion FROM AI.Legal_EnginePolicy policy JOIN AI.Legal_IntelligenceCapability capability ON capability.IntelligenceCapabilityId=policy.IntelligenceCapabilityId AND capability.IsDeleted=0 WHERE (policy.TenantId IS NULL OR policy.TenantId=@TenantId) AND policy.IsDeleted=0 ORDER BY capability.SortOrder,policy.PolicyCode,policy.VersionNumber DESC;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
    ...
    public async Task SaveEnginePolicyAsync(SaveIntelligenceEnginePolicyRequest request,CancellationToken cancellationToken=default)
    {
        const string sql="""
            ...
            IF @RowVersion IS NOT NULL
            BEGIN
              UPDATE AI.Legal_EnginePolicy SET IntelligenceCapabilityId=@IntelligenceCapabilityId,DisplayName=@DisplayName,Description=@Description,ExecutionModeCode=@ExecutionModeCode,ConfigurationJson=@ConfigurationJson,MinimumConfidence=@MinimumConfidence,RequiresHumanReview=@RequiresHumanReview,FailClosed=@FailClosed,EffectiveFromUtc=@EffectiveFromUtc,EffectiveToUtc=@EffectiveToUtc,IsActive=@IsActive,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE TenantId=@TenantId AND PolicyCode=@PolicyCode AND VersionNumber=@VersionNumber AND RowVersion=@RowVersion AND IsDeleted=0;
              IF @@ROWCOUNT=0 THROW 51000,'Engine policy changed before this update.',1;
            END
            ELSE
            BEGIN
              INSERT AI.Legal_EnginePolicy(TenantId,IntelligenceCapabilityId,PolicyCode,DisplayName,Description,ExecutionModeCode,ConfigurationJson,MinimumConfidence,RequiresHumanReview,FailClosed,EffectiveFromUtc,EffectiveToUtc,VersionNumber,IsActive,CreatedDateUtc,CreatedByUserId,IsDeleted)
              ...
              WHERE NOT EXISTS(SELECT 1 FROM AI.Legal_EnginePolicy WITH(UPDLOCK,HOLDLOCK) WHERE TenantId=@TenantId AND PolicyCode=@PolicyCode AND VersionNumber=@VersionNumber AND IsDeleted=0);
    ...
    {
        const string sql="""SELECT SafetyControlId,TenantId,ControlCode,DisplayName,Description,ControlTypeCode,EnforcementStageCode,ConfigurationJson,ViolationActionCode,RequiresHumanReview,SortOrder,IsActive,RowVersion FROM AI.Legal_SafetyControl WHERE (TenantId IS NULL OR TenantId=@TenantId) AND IsDeleted=0 ORDER BY SortOrder,DisplayName;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
    ...
    public async Task SaveSafetyControlAsync(SaveIntelligenceSafetyControlRequest request,CancellationToken cancellationToken=default)
    {
        const string sql="""
            IF @RowVersion IS NOT NULL
            BEGIN
              UPDATE AI.Legal_SafetyControl SET DisplayName=@DisplayName,Description=@Description,ControlTypeCode=@ControlTypeCode,EnforcementStageCode=@EnforcementStageCode,ConfigurationJson=@ConfigurationJson,ViolationActionCode=@ViolationActionCode,RequiresHumanReview=@RequiresHumanReview,SortOrder=@SortOrder,IsActive=@IsActive,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE TenantId=@TenantId AND ControlCode=@ControlCode AND RowVersion=@RowVersion AND IsDeleted=0;
              IF @@ROWCOUNT=0 THROW 51000,'Safety control changed before this update.',1;
            END
            ELSE
            BEGIN
              INSERT AI.Legal_SafetyControl(TenantId,ControlCode,DisplayName,Description,ControlTypeCode,EnforcementStageCode,ConfigurationJson,ViolationActionCode,RequiresHumanReview,SortOrder,IsActive,CreatedDateUtc,CreatedByUserId,IsDeleted)
              ...
              WHERE NOT EXISTS(SELECT 1 FROM AI.Legal_SafetyControl WITH(UPDLOCK,HOLDLOCK) WHERE TenantId=@TenantId AND ControlCode=@ControlCode AND IsDeleted=0);
    ...
    {
        const string from=""" FROM AI.Legal_IntelligenceFinding finding JOIN AI.Legal_IntelligenceCapability capability ON capability.IntelligenceCapabilityId=finding.IntelligenceCapabilityId WHERE finding.TenantId=@TenantId AND finding.IsDeleted=0 AND (@SearchTerm IS NULL OR finding.Title LIKE '%'+@SearchTerm+'%' OR finding.Summary LIKE '%'+@SearchTerm+'%' OR finding.Explanation LIKE '%'+@SearchTerm+'%') AND (@CapabilityCode IS NULL OR capability.CapabilityCode=@CapabilityCode) AND (@EntityTypeCode IS NULL OR finding.EntityTypeCode=@EntityTypeCode) AND (@EntityId IS NULL OR finding.EntityId=@EntityId) AND (@SeverityCode IS NULL OR finding.SeverityCode=@SeverityCode) AND (@StatusCode IS NULL OR finding.StatusCode=@StatusCode)""";
    ...
    {
        const string sql="""
            SELECT finding.IntelligenceFindingId,finding.TenantId,capability.CapabilityCode,capability.DisplayName CapabilityName,finding.EntityTypeCode,finding.EntityId,finding.FindingTypeCode,finding.SeverityCode,finding.StatusCode,finding.Title,finding.Summary,finding.Explanation,finding.Score,finding.Confidence,finding.RuleVersion,finding.DetectedDateUtc,finding.DueDateUtc,finding.ResolvedDateUtc,finding.ResolutionCode,finding.RowVersion FROM AI.Legal_IntelligenceFinding finding JOIN AI.Legal_IntelligenceCapability capability ON capability.IntelligenceCapabilityId=finding.IntelligenceCapabilityId WHERE finding.TenantId=@TenantId AND finding.IntelligenceFindingId=@FindingId AND finding.IsDeleted=0;
            SELECT evidence.FindingEvidenceId,evidence.EvidenceTypeCode,evidence.SourceModuleCode,evidence.SourceEntityTypeCode,evidence.SourceEntityId,evidence.SourceReference,evidence.Description,evidence.EvidenceValueJson,evidence.RelevanceScore FROM AI.Legal_FindingEvidence evidence JOIN AI.Legal_IntelligenceFinding finding ON finding.IntelligenceFindingId=evidence.IntelligenceFindingId AND finding.TenantId=@TenantId AND finding.IsDeleted=0 WHERE evidence.TenantId=@TenantId AND evidence.IntelligenceFindingId=@FindingId AND evidence.IsDeleted=0 ORDER BY evidence.RelevanceScore DESC,evidence.CreatedDateUtc;
            """;
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
    ...
    public async Task<EntityRelationshipGraphDto> GetRelationshipGraphAsync(RelationshipQuery query,CancellationToken cancellationToken=default)
    {
        const string sql="""
            IF NOT EXISTS
            (
              SELECT 1 FROM AI.Legal_SearchDocument document
              JOIN AI.Legal_SearchPermission permission ON permission.SearchDocumentId=document.SearchDocumentId AND permission.TenantId=@TenantId AND permission.PermissionCode=N'READ' AND permission.IsDeleted=0
            ...
            (
              ...
              FROM AI.Legal_EntityRelationship relationship
    ...
    {
        const string sql="""UPDATE AI.Legal_BusinessSignal SET StatusCode=CASE WHEN @DecisionCode=N'REOPEN' THEN N'OPEN' ELSE @DecisionCode END,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE TenantId=@TenantId AND BusinessSignalId=@BusinessSignalId AND RowVersion=@RowVersion AND IsDeleted=0; IF @@ROWCOUNT=0 THROW 51000,'Business signal changed before this decision.',1;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
        await connection.ExecuteAsync(new CommandDefinition(sql,request,cancellationToken:cancellationToken));
    }
    ...
    public async Task<InsuranceReasoningResponse> ExecuteReasoningAsync(InsuranceReasoningRequest request,IReadOnlyCollection<SemanticConceptMatchDto> concepts,CancellationToken cancellationToken=default)
    {
        ...
        const string sql="""
            IF EXISTS(SELECT 1 FROM AI.Legal_ReasoningSession WHERE TenantId=@TenantId AND CorrelationId=@CorrelationId AND IsDeleted=0)
            BEGIN
              SELECT ReasoningSessionId FROM AI.Legal_ReasoningSession WHERE TenantId=@TenantId AND CorrelationId=@CorrelationId AND RequestedByUserId=@UserId AND IsDeleted=0;
              RETURN;
            END;
            IF NOT EXISTS(SELECT 1 FROM AI.Legal_SearchDocument document JOIN AI.Legal_SearchPermission permission ON permission.SearchDocumentId=document.SearchDocumentId AND permission.TenantId=@TenantId AND permission.PermissionCode=N'READ' AND permission.IsDeleted=0 WHERE document.TenantId=@TenantId AND document.EntityTypeCode=@EntityTypeCode AND document.EntityId=@EntityId AND document.IsDeleted=0 AND ((permission.PrincipalTypeCode=N'USER' AND permission.PrincipalId=@UserId) OR (permission.PrincipalTypeCode=N'ROLE' AND EXISTS(SELECT 1 FROM IAM.UserRole userRole WHERE userRole.TenantId=@TenantId AND userRole.UserId=@UserId AND userRole.RoleId=permission.PrincipalId AND userRole.IsActive=1 AND userRole.IsDeleted=0)))) THROW 51000,'The requested entity was not found or is not authorized.',1;
            DECLARE @SessionId UNIQUEIDENTIFIER=NEWID();
            DECLARE @PolicyId UNIQUEIDENTIFIER=(SELECT TOP(1) policy.EnginePolicyId FROM AI.Legal_EnginePolicy policy JOIN AI.Legal_IntelligenceCapability capability ON capability.IntelligenceCapabilityId=policy.IntelligenceCapabilityId WHERE capability.CapabilityCode=N'INSURANCE_REASONING' AND capability.IsDeleted=0 AND policy.IsActive=1 AND policy.IsDeleted=0 AND (policy.TenantId=@TenantId OR policy.TenantId IS NULL) AND policy.EffectiveFromUtc<=SYSUTCDATETIME() AND (policy.EffectiveToUtc IS NULL OR policy.EffectiveToUtc>SYSUTCDATETIME()) ORDER BY CASE WHEN policy.TenantId=@TenantId THEN 0 ELSE 1 END,policy.VersionNumber DESC);
            IF @PolicyId IS NULL THROW 51000,'No active insurance reasoning policy is configured.',1;
            INSERT AI.Legal_ReasoningSession(ReasoningSessionId,TenantId,RequestedByUserId,EntityTypeCode,EntityId,Question,IntentCode,StatusCode,CorrelationId,EnginePolicyId,PermissionSnapshotHash,StartedDateUtc,RequiresHumanReview,CreatedDateUtc,CreatedByUserId,IsDeleted) VALUES(@SessionId,@TenantId,@UserId,@EntityTypeCode,@EntityId,@Question,@IntentCode,N'PROCESSING',@CorrelationId,@PolicyId,@PermissionHash,SYSUTCDATETIME(),0,SYSUTCDATETIME(),@UserId,0);
            INSERT AI.Legal_ReasoningEvidence(ReasoningEvidenceId,TenantId,ReasoningSessionId,EvidenceTypeCode,SourceModuleCode,SourceEntityTypeCode,SourceEntityId,SourceReference,Title,Summary,EvidenceValueJson,RelevanceScore,IsAuthoritative,CreatedDateUtc,CreatedByUserId,IsDeleted)
            ...
            FROM AI.Legal_SearchDocument document WHERE document.TenantId=@TenantId AND document.EntityTypeCode=@EntityTypeCode AND document.EntityId=@EntityId AND document.IsDeleted=0;
            INSERT AI.Legal_ReasoningEvidence(ReasoningEvidenceId,TenantId,ReasoningSessionId,EvidenceTypeCode,SourceModuleCode,SourceEntityTypeCode,SourceEntityId,SourceReference,Title,Summary,EvidenceValueJson,RelevanceScore,IsAuthoritative,CreatedDateUtc,CreatedByUserId,IsDeleted)
            SELECT NEWID(),@TenantId,@SessionId,N'FINDING',capability.OwningModuleCode,finding.EntityTypeCode,finding.EntityId,N'intelligence-finding:'+CONVERT(nvarchar(36),finding.IntelligenceFindingId),finding.Title,finding.Summary,NULL,COALESCE(finding.Confidence,0.7500),1,SYSUTCDATETIME(),@UserId,0 FROM AI.Legal_IntelligenceFinding finding JOIN AI.Legal_IntelligenceCapability capability ON capability.IntelligenceCapabilityId=finding.IntelligenceCapabilityId WHERE finding.TenantId=@TenantId AND finding.EntityTypeCode=@EntityTypeCode AND finding.EntityId=@EntityId AND finding.StatusCode IN(N'OPEN',N'REVIEW_REQUIRED',N'IN_REVIEW') AND finding.IsDeleted=0;
            INSERT AI.Legal_ReasoningEvidence(ReasoningEvidenceId,TenantId,ReasoningSessionId,EvidenceTypeCode,SourceModuleCode,SourceEntityTypeCode,SourceEntityId,SourceReference,Title,Summary,EvidenceValueJson,RelevanceScore,IsAuthoritative,CreatedDateUtc,CreatedByUserId,IsDeleted)
            ...
            INSERT AI.Legal_ReasoningEvidence(ReasoningEvidenceId,TenantId,ReasoningSessionId,EvidenceTypeCode,SourceModuleCode,SourceEntityTypeCode,SourceEntityId,SourceReference,Title,Summary,EvidenceValueJson,RelevanceScore,IsAuthoritative,CreatedDateUtc,CreatedByUserId,IsDeleted)
            SELECT TOP(10) NEWID(),@TenantId,@SessionId,N'SIMILAR_HISTORY',N'AI',N'SUBMISSION',similarity.SimilarEntityId,N'AI.Legal_EntitySimilarity:'+CONVERT(nvarchar(36),similarity.EntitySimilarityId),N'Similar historical submission',CONCAT(N'Similarity ',FORMAT(similarity.SimilarityScore,N'P0'),N' using ',similarity.SimilarityModelCode,N' version ',similarity.SimilarityModelVersion,N'.'),similarity.FeatureEvidenceJson,similarity.SimilarityScore,0,SYSUTCDATETIME(),@UserId,0 FROM Submissions.PolicyBindTransaction bind JOIN AI.Legal_EntitySimilarity similarity ON similarity.TenantId=bind.TenantId AND similarity.EntityTypeCode=N'SUBMISSION' AND similarity.SourceEntityId=bind.SubmissionId AND similarity.IsDeleted=0 AND (similarity.ExpiresDateUtc IS NULL OR similarity.ExpiresDateUtc>SYSUTCDATETIME()) WHERE bind.TenantId=@TenantId AND bind.IsDeleted=0 AND ((@EntityTypeCode=N'SUBMISSION' AND bind.SubmissionId=@EntityId) OR (@EntityTypeCode=N'BIND_TRANSACTION' AND bind.PolicyBindTransactionId=@EntityId)) AND EXISTS(SELECT 1 FROM AI.Legal_SearchDocument document JOIN AI.Legal_SearchPermission permission ON permission.SearchDocumentId=document.SearchDocumentId AND permission.TenantId=@TenantId AND permission.PermissionCode=N'READ' AND permission.IsDeleted=0 WHERE document.TenantId=@TenantId AND document.EntityTypeCode=N'SUBMISSION' AND document.EntityId=similarity.SimilarEntityId AND document.IsDeleted=0 AND ((permission.PrincipalTypeCode=N'USER' AND permission.PrincipalId=@UserId) OR (permission.PrincipalTypeCode=N'ROLE' AND EXISTS(SELECT 1 FROM IAM.UserRole userRole WHERE userRole.TenantId=@TenantId AND userRole.UserId=@UserId AND userRole.RoleId=permission.PrincipalId AND userRole.IsActive=1 AND userRole.IsDeleted=0)))) ORDER BY similarity.SimilarityScore DESC;
            INSERT AI.Legal_ReasoningConclusion(ReasoningConclusionId,TenantId,ReasoningSessionId,ConclusionCode,SequenceNumber,Title,Explanation,RuleCode,RuleVersion,Confidence,IsBlocking,CanBeWaived,WaiverPermissionCode,CreatedDateUtc,CreatedByUserId,IsDeleted)
            SELECT NEWID(),@TenantId,@SessionId,finding.FindingTypeCode,ROW_NUMBER() OVER(ORDER BY CASE finding.SeverityCode WHEN N'CRITICAL' THEN 1 WHEN N'HIGH' THEN 2 WHEN N'MEDIUM' THEN 3 ELSE 4 END,finding.DetectedDateUtc),finding.Title,finding.Explanation,policy.PolicyCode,finding.RuleVersion,COALESCE(finding.Confidence,0.7500),CASE WHEN finding.SeverityCode IN(N'CRITICAL',N'HIGH') THEN 1 ELSE 0 END,0,NULL,SYSUTCDATETIME(),@UserId,0 FROM AI.Legal_IntelligenceFinding finding LEFT JOIN AI.Legal_EnginePolicy policy ON policy.EnginePolicyId=finding.EnginePolicyId WHERE finding.TenantId=@TenantId AND finding.EntityTypeCode=@EntityTypeCode AND finding.EntityId=@EntityId AND finding.StatusCode IN(N'OPEN',N'REVIEW_REQUIRED',N'IN_REVIEW') AND finding.IsDeleted=0;
            DECLARE @ConclusionOffset INT=(SELECT COUNT(1) FROM AI.Legal_ReasoningConclusion WHERE TenantId=@TenantId AND ReasoningSessionId=@SessionId AND IsDeleted=0);
            INSERT AI.Legal_ReasoningConclusion(ReasoningConclusionId,TenantId,ReasoningSessionId,ConclusionCode,SequenceNumber,Title,Explanation,RuleCode,RuleVersion,Confidence,IsBlocking,CanBeWaived,WaiverPermissionCode,CreatedDateUtc,CreatedByUserId,IsDeleted)
            ...
            IF NOT EXISTS(SELECT 1 FROM AI.Legal_ReasoningConclusion WHERE TenantId=@TenantId AND ReasoningSessionId=@SessionId AND IsDeleted=0)
              INSERT AI.Legal_ReasoningConclusion(ReasoningConclusionId,TenantId,ReasoningSessionId,ConclusionCode,SequenceNumber,Title,Explanation,Confidence,IsBlocking,CanBeWaived,CreatedDateUtc,CreatedByUserId,IsDeleted) VALUES(NEWID(),@TenantId,@SessionId,N'NO_ACTIVE_FINDINGS',1,N'No active governed findings',N'No active risk, compliance, or workflow findings are currently recorded for this authorized entity. This does not replace required operational validation.',1.0000,0,0,SYSUTCDATETIME(),@UserId,0);
            INSERT AI.Legal_ReasoningAction(ReasoningActionId,TenantId,ReasoningSessionId,SequenceNumber,ActionCode,DisplayName,Description,TargetRoute,RequiredPermissionCode,RequiresConfirmation,IsAvailable,UnavailableReason,CreatedDateUtc,CreatedByUserId,IsDeleted)
            ...
            DECLARE @Confidence DECIMAL(5,4)=(SELECT MIN(Confidence) FROM AI.Legal_ReasoningConclusion WHERE TenantId=@TenantId AND ReasoningSessionId=@SessionId AND IsDeleted=0);
            DECLARE @RequiresReview BIT=CASE WHEN @Confidence<(SELECT TOP(1) MinimumConfidence FROM AI.Legal_EnginePolicy WHERE EnginePolicyId=@PolicyId) OR EXISTS(SELECT 1 FROM AI.Legal_ReasoningConclusion WHERE TenantId=@TenantId AND ReasoningSessionId=@SessionId AND IsBlocking=1 AND IsDeleted=0) THEN 1 ELSE 0 END;
            UPDATE AI.Legal_ReasoningSession SET StatusCode=N'COMPLETED',CompletedDateUtc=SYSUTCDATETIME(),Confidence=@Confidence,RequiresHumanReview=@RequiresReview,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@UserId WHERE ReasoningSessionId=@SessionId AND TenantId=@TenantId;
            SELECT @SessionId;
            """;
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
        ...
        if(concepts.Count>0)
        {
            const string conceptSql="""INSERT AI.Legal_ReasoningEvidence(ReasoningEvidenceId,TenantId,ReasoningSessionId,EvidenceTypeCode,SourceModuleCode,SourceEntityTypeCode,SourceEntityId,SourceReference,Title,Summary,EvidenceValueJson,RelevanceScore,IsAuthoritative,CreatedDateUtc,CreatedByUserId,IsDeleted) SELECT NEWID(),@TenantId,@ReasoningSessionId,N'KNOWLEDGE_CONCEPT',N'Knowledge',N'KNOWLEDGE_CONCEPT',@ConceptId,N'knowledge-concept:'+CONVERT(nvarchar(36),@ConceptId),@PreferredLabel,N'Approved canonical concept matched through the Knowledge semantic contract.',@EvidenceValueJson,@Score,1,SYSUTCDATETIME(),@UserId,0 WHERE NOT EXISTS(SELECT 1 FROM AI.Legal_ReasoningEvidence WHERE TenantId=@TenantId AND ReasoningSessionId=@ReasoningSessionId AND SourceEntityId=@ConceptId AND EvidenceTypeCode=N'KNOWLEDGE_CONCEPT' AND IsDeleted=0);""";
    ...
    {
        const string sql="""
            SELECT session.ReasoningSessionId,session.EntityTypeCode,session.EntityId,session.Question,session.IntentCode,session.StatusCode,session.CorrelationId,session.Confidence,session.RequiresHumanReview,session.StartedDateUtc,session.CompletedDateUtc FROM AI.Legal_ReasoningSession session WHERE session.TenantId=@TenantId AND session.RequestedByUserId=@UserId AND session.ReasoningSessionId=@ReasoningSessionId AND session.IsDeleted=0;
            SELECT ReasoningEvidenceId,EvidenceTypeCode,SourceModuleCode,SourceEntityTypeCode,SourceEntityId,SourceReference,Title,Summary,EvidenceValueJson,RelevanceScore,IsAuthoritative FROM AI.Legal_ReasoningEvidence WHERE TenantId=@TenantId AND ReasoningSessionId=@ReasoningSessionId AND IsDeleted=0 ORDER BY RelevanceScore DESC,CreatedDateUtc;
            SELECT ReasoningConclusionId,ConclusionCode,SequenceNumber,Title,Explanation,RuleCode,RuleVersion,Confidence,IsBlocking,CanBeWaived,WaiverPermissionCode FROM AI.Legal_ReasoningConclusion WHERE TenantId=@TenantId AND ReasoningSessionId=@ReasoningSessionId AND IsDeleted=0 ORDER BY SequenceNumber;
            SELECT ReasoningActionId,SequenceNumber,ActionCode,DisplayName,Description,TargetRoute,RequiredPermissionCode,RequiresConfirmation,IsAvailable,UnavailableReason FROM AI.Legal_ReasoningAction WHERE TenantId=@TenantId AND ReasoningSessionId=@ReasoningSessionId AND IsDeleted=0 ORDER BY SequenceNumber;
            """;
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
