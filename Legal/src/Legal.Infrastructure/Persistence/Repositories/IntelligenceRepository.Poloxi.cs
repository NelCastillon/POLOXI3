...
(
    ...
    FROM POLOXI.Legal_Capability capability WHERE capability.IsActive=1 AND capability.IsDeleted=0 AND (capability.TenantId=@TenantId OR capability.TenantId IS NULL)
)
...
FROM POLOXI.Legal_Hierarchy WHERE TenantId=@TenantId AND QuerySignature=@QuerySignature AND StatusCode=N'VALIDATED' AND IsDeleted=0 AND (ExpiresDateUtc IS NULL OR ExpiresDateUtc>SYSUTCDATETIME()) ORDER BY VersionNumber DESC;
...
FROM POLOXI.Legal_HierarchyBranch branch JOIN POLOXI.Legal_Hierarchy hierarchy ON hierarchy.HierarchyId=branch.HierarchyId
...
AND hierarchy.VersionNumber=(SELECT MAX(versioned.VersionNumber) FROM POLOXI.Legal_Hierarchy versioned WHERE versioned.TenantId=@TenantId AND versioned.QuerySignature=@QuerySignature AND versioned.StatusCode=N'VALIDATED' AND versioned.IsDeleted=0 AND (versioned.ExpiresDateUtc IS NULL OR versioned.ExpiresDateUtc>SYSUTCDATETIME())) ORDER BY branch.SortOrder;
""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
...
DECLARE @VersionNumber INT=COALESCE((SELECT MAX(VersionNumber)+1 FROM POLOXI.Legal_Hierarchy WITH(UPDLOCK,HOLDLOCK) WHERE TenantId=@TenantId AND QuerySignature=@QuerySignature),1);
INSERT POLOXI.Legal_Hierarchy(HierarchyId,TenantId,QuerySignature,ConceptCode,DisplayName,NormalizedQuery,VersionNumber,StatusCode,GeneratedByProviderCode,GeneratedByModelCode,Confidence,UsageCount,SuccessfulUsageCount,ExpiresDateUtc,CreatedDateUtc,CreatedByUserId,IsDeleted)
...
INSERT POLOXI.Legal_HierarchyBranch(HierarchyBranchId,TenantId,HierarchyId,ParentHierarchyBranchId,BranchCode,DisplayName,ProposedCondition,CapabilityCode,ValidationStatusCode,ValidationMessage,SearchText,OrderByRecency,Confidence,SortOrder,CreatedDateUtc,CreatedByUserId,IsDeleted)
...
""";
    ...
    {
        const string sql="""DECLARE @Id UNIQUEIDENTIFIER=NEWID();INSERT POLOXI.Legal_Execution(PoloxiExecutionId,TenantId,HierarchyId,UserId,QueryText,CorrelationId,StatusCode,WasHierarchyReused,ValidBranchCount,UnsupportedBranchCount,ResultCount,Confidence,ExplanationStatusCode,StartedDateUtc,CreatedDateUtc,CreatedByUserId,IsDeleted) VALUES(@Id,@TenantId,@HierarchyId,@UserId,@QueryText,@CorrelationId,N'RUNNING',@WasHierarchyReused,@ValidBranchCount,@UnsupportedBranchCount,0,@Confidence,N'NOT_REQUESTED',SYSUTCDATETIME(),SYSUTCDATETIME(),@UserId,0);UPDATE POLOXI.Legal_Hierarchy SET UsageCount=UsageCount+1,LastUsedDateUtc=SYSUTCDATETIME(),ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@UserId WHERE TenantId=@TenantId AND HierarchyId=@HierarchyId AND IsDeleted=0;SELECT @Id;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
    ...
    public async Task CompletePoloxiExecutionAsync(Guid tenantId,Guid userId,Guid poloxiExecutionId,Guid hierarchyId,IReadOnlyCollection<PoloxiEvidenceDto> evidence,string explanationStatusCode,string? explanation,long durationMilliseconds,CancellationToken cancellationToken=default)
    {
        const string sql="""
SET XACT_ABORT ON;BEGIN TRANSACTION;
UPDATE POLOXI.Legal_Execution SET StatusCode=N'COMPLETED',ResultCount=@ResultCount,ExplanationStatusCode=@ExplanationStatusCode,Explanation=@Explanation,DurationMilliseconds=@DurationMilliseconds,CompletedDateUtc=SYSUTCDATETIME(),ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@UserId WHERE TenantId=@TenantId AND PoloxiExecutionId=@PoloxiExecutionId AND IsDeleted=0;
INSERT POLOXI.Legal_ExecutionEvidence(ExecutionEvidenceId,TenantId,PoloxiExecutionId,HierarchyBranchId,SearchDocumentId,EntityTypeCode,EntityId,SourceReference,Title,Excerpt,RelevanceScore,RankNumber,CreatedDateUtc,CreatedByUserId,IsDeleted)
...
UPDATE POLOXI.Legal_Hierarchy SET SuccessfulUsageCount=SuccessfulUsageCount+CASE WHEN @ResultCount>0 THEN 1 ELSE 0 END,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@UserId WHERE TenantId=@TenantId AND HierarchyId=@HierarchyId AND IsDeleted=0;
COMMIT;
""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);
