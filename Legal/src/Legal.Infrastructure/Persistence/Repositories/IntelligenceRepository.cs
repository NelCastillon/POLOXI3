...
public sealed partial class IntelligenceRepository(ISqlConnectionFactory connectionFactory):IIntelligenceRepository,IRecommendationGenerationRepository
{
    ...
    {
        const string sql="""SELECT ProviderId,ProviderCode,DisplayName,ProviderTypeCode,SupportsChat,SupportsEmbeddings,SupportsVision,SupportsStructuredOutput,Priority,IsActive,RowVersion FROM AI.Legal_Provider WHERE (TenantId IS NULL OR TenantId=@TenantId) AND IsDeleted=0 ORDER BY Priority,DisplayName;""";
    ...
    public async Task<IntelligenceSearchConfiguration> GetSearchConfigurationAsync(Guid tenantId,CancellationToken cancellationToken=default)
    {
        const string sql="""
            SELECT
              ...
              CONVERT(bit,COALESCE(TRY_CONVERT(bit,(SELECT TOP(1) COALESCE(SettingValue,DefaultValue) FROM Core.ConfigurationSetting WHERE SettingKey=N'Intelligence.QuickSearch.EnableIntelligentFallback' AND IsDeleted=0 AND (TenantId=@TenantId OR TenantId IS NULL) ORDER BY CASE WHEN TenantId=@TenantId THEN 0 ELSE 1 END)),1)) EnableQuickSearchIntelligentFallback,
              COALESCE(TRY_CONVERT(int,(SELECT TOP(1) COALESCE(SettingValue,DefaultValue) FROM Core.ConfigurationSetting WHERE SettingKey=N'Intelligence.QuickSearch.FastPathMinimumResults' AND IsDeleted=0 AND (TenantId=@TenantId OR TenantId IS NULL) ORDER BY CASE WHEN TenantId=@TenantId THEN 0 ELSE 1 END)),3) QuickSearchFastPathMinimumResults,
              COALESCE(TRY_CONVERT(decimal(9,6),(SELECT TOP(1) COALESCE(SettingValue,DefaultValue) FROM Core.ConfigurationSetting WHERE SettingKey=N'Intelligence.QuickSearch.FastPathMinimumScore' AND IsDeleted=0 AND (TenantId=@TenantId OR TenantId IS NULL) ORDER BY CASE WHEN TenantId=@TenantId THEN 0 ELSE 1 END)),.70) QuickSearchFastPathMinimumScore;
            """;
        ...
        return new(new(row.KeywordWeight,row.SemanticWeight,row.FuzzyWeight,row.RelationshipWeight,row.RecencyWeight,row.BusinessPriorityWeight),Math.Clamp(row.RecencyWindowDays,1,3650),Math.Clamp(row.MaximumRelationshipResults,0,100),Math.Clamp(row.MinimumUnifiedScore,0,1),row.EnableRules,row.EnableRelationships,row.EnableAiSummary,row.EnableLlmIntentFallback,Math.Clamp(row.LlmIntentMinimumConfidence,0,1),Math.Clamp(row.LlmIntentTimeoutSeconds,1,60),row.EnableQuickSearchIntelligentFallback,Math.Clamp(row.QuickSearchFastPathMinimumResults,1,12),Math.Clamp(row.QuickSearchFastPathMinimumScore,0,1));
    }
    ...
    public async Task<IReadOnlyCollection<IntelligenceSearchIntentPatternDto>> GetSearchIntentPatternsAsync(Guid tenantId,CancellationToken cancellationToken=default)
    {
        const string sql="""
            ...
            (
                ...
                FROM AI.Legal_SearchIntentPattern pattern
            ...
            JOIN AI.Legal_SearchIntentPatternPhrase phrase ON phrase.SearchIntentPatternId=pattern.SearchIntentPatternId AND phrase.IsActive=1 AND phrase.IsDeleted=0
    ...
    public async Task<IReadOnlyCollection<IntelligenceSearchResultDto>> GetAuthorizedSearchDocumentsAsync(IntelligenceSearchRequest request,IReadOnlyCollection<IntelligenceSearchEntityKey> entities,CancellationToken cancellationToken=default)
    {
        ...
        const string sql="""
            ...
            FROM AI.Legal_SearchDocument document
    ...
    public async Task<IReadOnlyCollection<IntelligenceSearchResultDto>> GetRelatedSearchDocumentsAsync(IntelligenceSearchRequest request,IReadOnlyCollection<IntelligenceSearchEntityKey> sources,int maximumResults,CancellationToken cancellationToken=default)
    {
        ...
        const string sql="""
            ...
            JOIN AI.Legal_EntityRelationship relationship ON relationship.TenantId=@TenantId AND relationship.SourceEntityTypeCode=source.EntityTypeCode AND relationship.SourceEntityId=source.EntityId AND relationship.IsDeleted=0 AND (relationship.EffectiveFromUtc IS NULL OR relationship.EffectiveFromUtc<=SYSUTCDATETIME()) AND (relationship.EffectiveToUtc IS NULL OR relationship.EffectiveToUtc>SYSUTCDATETIME())
            JOIN AI.Legal_SearchDocument document ON document.TenantId=@TenantId AND document.EntityTypeCode=relationship.TargetEntityTypeCode AND document.EntityId=relationship.TargetEntityId AND document.IsDeleted=0
            LEFT JOIN Search.EntityProjection projection ON projection.TenantId=document.TenantId AND projection.EntityTypeCode=document.EntityTypeCode AND projection.EntityId=document.EntityId AND projection.IsActive=1 AND projection.IsDeleted=0
    ...
    {
        const string sql="""SELECT model.ModelDeploymentId,model.ProviderId,provider.ProviderCode,model.ModelCode,model.DeploymentName,model.ModelFamily,model.CapabilityCode,model.ContextWindowTokens,model.MaximumOutputTokens,model.InputCostPerMillionTokens,model.OutputCostPerMillionTokens,model.CurrencyCode,model.Priority,model.IsFallback,model.IsActive,model.RowVersion FROM AI.Legal_ModelDeployment model JOIN AI.Legal_Provider provider ON provider.ProviderId=model.ProviderId AND provider.IsDeleted=0 WHERE (model.TenantId IS NULL OR model.TenantId=@TenantId) AND model.IsDeleted=0 ORDER BY model.Priority,model.DeploymentName;""";
    ...
    {
        const string sql="""SELECT FeaturePolicyId,TenantId,FeatureCode,ModuleCode,PrimaryModelDeploymentId,FallbackModelDeploymentId,Temperature,MaximumInputTokens,MaximumOutputTokens,TimeoutSeconds,DailyCostLimit,MonthlyCostLimit,MinimumConfidence,RequiresHumanReview,IsEnabled,RowVersion FROM AI.Legal_FeaturePolicy WHERE TenantId=@TenantId AND IsDeleted=0 ORDER BY ModuleCode,FeatureCode;""";
    ...
    public async Task SaveProviderAsync(SaveAiProviderRequest request,CancellationToken cancellationToken=default)
    {
        const string sql="""IF @RowVersion IS NOT NULL BEGIN UPDATE AI.Legal_Provider SET DisplayName=@DisplayName,ProviderTypeCode=@ProviderTypeCode,SupportsChat=@SupportsChat,SupportsEmbeddings=@SupportsEmbeddings,SupportsVision=@SupportsVision,SupportsStructuredOutput=@SupportsStructuredOutput,Priority=@Priority,IsActive=@IsActive,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE ProviderCode=@ProviderCode AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0 AND RowVersion=@RowVersion; IF @@ROWCOUNT=0 THROW 51000,'AI provider changed before this update.',1; END ELSE BEGIN INSERT AI.Legal_Provider(ProviderId,TenantId,ProviderCode,DisplayName,ProviderTypeCode,SupportsChat,SupportsEmbeddings,SupportsVision,SupportsStructuredOutput,Priority,IsActive,CreatedDateUtc,CreatedByUserId,IsDeleted) SELECT NEWID(),@TenantId,@ProviderCode,@DisplayName,@ProviderTypeCode,@SupportsChat,@SupportsEmbeddings,@SupportsVision,@SupportsStructuredOutput,@Priority,@IsActive,SYSUTCDATETIME(),@ActorUserId,0 WHERE NOT EXISTS(SELECT 1 FROM AI.Legal_Provider WITH(UPDLOCK,HOLDLOCK) WHERE ProviderCode=@ProviderCode AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0); IF @@ROWCOUNT=0 THROW 51000,'AI provider already exists; reload it before updating.',1; END;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);await connection.ExecuteAsync(new CommandDefinition(sql,request,cancellationToken:cancellationToken));
    }

    public async Task SaveModelDeploymentAsync(SaveAiModelDeploymentRequest request,CancellationToken cancellationToken=default)
    {
        const string sql="""IF NOT EXISTS(SELECT 1 FROM AI.Legal_Provider WHERE ProviderId=@ProviderId AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0) THROW 51000,'AI provider was not found.',1; IF @RowVersion IS NOT NULL BEGIN UPDATE AI.Legal_ModelDeployment SET ProviderId=@ProviderId,DeploymentName=@DeploymentName,ModelFamily=@ModelFamily,CapabilityCode=@CapabilityCode,ContextWindowTokens=@ContextWindowTokens,MaximumOutputTokens=@MaximumOutputTokens,InputCostPerMillionTokens=@InputCostPerMillionTokens,OutputCostPerMillionTokens=@OutputCostPerMillionTokens,CurrencyCode=@CurrencyCode,Priority=@Priority,IsFallback=@IsFallback,IsActive=@IsActive,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE ModelCode=@ModelCode AND ProviderId=@ProviderId AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0 AND RowVersion=@RowVersion; IF @@ROWCOUNT=0 THROW 51000,'AI model deployment changed before this update.',1; END ELSE BEGIN INSERT AI.Legal_ModelDeployment(ModelDeploymentId,TenantId,ProviderId,ModelCode,DeploymentName,ModelFamily,CapabilityCode,ContextWindowTokens,MaximumOutputTokens,InputCostPerMillionTokens,OutputCostPerMillionTokens,CurrencyCode,Priority,IsFallback,IsActive,CreatedDateUtc,CreatedByUserId,IsDeleted) SELECT NEWID(),@TenantId,@ProviderId,@ModelCode,@DeploymentName,@ModelFamily,@CapabilityCode,@ContextWindowTokens,@MaximumOutputTokens,@InputCostPerMillionTokens,@OutputCostPerMillionTokens,@CurrencyCode,@Priority,@IsFallback,@IsActive,SYSUTCDATETIME(),@ActorUserId,0 WHERE NOT EXISTS(SELECT 1 FROM AI.Legal_ModelDeployment WITH(UPDLOCK,HOLDLOCK) WHERE ModelCode=@ModelCode AND ProviderId=@ProviderId AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0); IF @@ROWCOUNT=0 THROW 51000,'AI model deployment already exists; reload it before updating.',1; END;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);await connection.ExecuteAsync(new CommandDefinition(sql,request,cancellationToken:cancellationToken));
    }

    public async Task DeleteProviderAsync(Guid tenantId,string providerCode,Guid actorUserId,CancellationToken cancellationToken=default)
    {
        const string sql="""IF EXISTS(SELECT 1 FROM AI.Legal_ModelDeployment model JOIN AI.Legal_Provider provider ON provider.ProviderId=model.ProviderId WHERE provider.ProviderCode=@ProviderCode AND (provider.TenantId=@TenantId OR provider.TenantId IS NULL) AND model.IsDeleted=0) THROW 51000,'Provider has active model deployments; delete or reassign them first.',1; UPDATE AI.Legal_Provider SET IsDeleted=1,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE ProviderCode=@ProviderCode AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0; IF @@ROWCOUNT=0 THROW 51000,'Provider was not found.',1;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);await connection.ExecuteAsync(new CommandDefinition(sql,new{TenantId=tenantId,ProviderCode=providerCode,ActorUserId=actorUserId},cancellationToken:cancellationToken));
    }

    public async Task DeleteModelDeploymentAsync(Guid tenantId,string modelCode,Guid actorUserId,CancellationToken cancellationToken=default)
    {
        const string sql="""IF EXISTS(SELECT 1 FROM AI.Legal_FeaturePolicy policy JOIN AI.Legal_ModelDeployment model ON model.ModelDeploymentId IN(policy.PrimaryModelDeploymentId,policy.FallbackModelDeploymentId) WHERE model.ModelCode=@ModelCode AND (model.TenantId=@TenantId OR model.TenantId IS NULL) AND policy.IsDeleted=0) THROW 51000,'Model deployment is referenced by feature policies; update them first.',1; UPDATE AI.Legal_ModelDeployment SET IsDeleted=1,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE ModelCode=@ModelCode AND (TenantId=@TenantId OR TenantId IS NULL) AND IsDeleted=0; IF @@ROWCOUNT=0 THROW 51000,'Model deployment was not found.',1;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);await connection.ExecuteAsync(new CommandDefinition(sql,new{TenantId=tenantId,ModelCode=modelCode,ActorUserId=actorUserId},cancellationToken:cancellationToken));
    }

    public async Task DeleteFeaturePolicyAsync(Guid tenantId,string featureCode,Guid actorUserId,CancellationToken cancellationToken=default)
    {
        const string sql="""UPDATE AI.Legal_FeaturePolicy SET IsDeleted=1,ModifiedDateUtc=SYSUTCDATETIME(),ModifiedByUserId=@ActorUserId WHERE TenantId=@TenantId AND FeatureCode=@FeatureCode AND IsDeleted=0; IF @@ROWCOUNT=0 THROW 51000,'Feature policy was not found.',1;""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);await connection.ExecuteAsync(new CommandDefinition(sql,new{TenantId=tenantId,FeatureCode=featureCode,ActorUserId=actorUserId},cancellationToken:cancellationToken));
    }
    ...
    {
        const string sql="""SELECT execution.ExecutionId,execution.TenantId,execution.FeatureCode,execution.ModuleCode,execution.EntityTypeCode,execution.EntityId,execution.StatusCode,provider.ProviderCode,model.ModelCode,execution.PromptVersion,execution.DurationMilliseconds,execution.InputTokenCount,execution.OutputTokenCount,execution.EstimatedCost,execution.CurrencyCode,execution.Confidence,execution.GroundingSourceCount,execution.RequestedByUserId,execution.StartedDateUtc,execution.CompletedDateUtc,execution.CorrelationId,execution.ErrorCode,execution.ErrorMessage,execution.RowVersion FROM AI.Legal_Execution execution LEFT JOIN AI.Legal_Provider provider ON provider.ProviderId=execution.ProviderId LEFT JOIN AI.Legal_ModelDeployment model ON model.ModelDeploymentId=execution.ModelDeploymentId WHERE execution.TenantId=@TenantId AND execution.ExecutionId=@ExecutionId AND execution.IsDeleted=0; SELECT ExecutionGroundingSourceId GroundingSourceId,SourceTypeCode,CAST(NULL AS NVARCHAR(100)) SourceEntityTypeCode,SourceEntityId,SourceReference,Title,RelevanceScore FROM AI.Legal_ExecutionGroundingSource WHERE TenantId=@TenantId AND ExecutionId=@ExecutionId AND IsDeleted=0 ORDER BY RelevanceScore DESC; SELECT ExecutionFeedbackId FeedbackId,FeedbackTypeCode,Rating,CorrectionReference,Comment,DecisionCode,ReviewedByUserId,ReviewedDateUtc,CreatedDateUtc FROM AI.Legal_ExecutionFeedback WHERE TenantId=@TenantId AND ExecutionId=@ExecutionId AND IsDeleted=0 ORDER BY CreatedDateUtc DESC;""";
    ...
    {
        const string sql="""IF NOT EXISTS(SELECT 1 FROM AI.Legal_Execution WHERE TenantId=@TenantId AND ExecutionId=@ExecutionId AND IsDeleted=0) THROW 51000,'AI execution was not found for tenant.',1; INSERT AI.Legal_ExecutionFeedback(TenantId,ExecutionId,FeedbackTypeCode,Rating,CorrectionReference,Comment,CreatedDateUtc,CreatedByUserId,IsDeleted) VALUES(@TenantId,@ExecutionId,@FeedbackTypeCode,@Rating,@CorrectionReference,@Comment,SYSUTCDATETIME(),@ActorUserId,0);""";
        using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);await connection.ExecuteAsync(new CommandDefinition(sql,request,cancellationToken:cancellationToken));
    }
    ...
    {
        const string sql="""SELECT RecommendationTypeId,TypeCode,DisplayName,Description,TargetModuleCode,DefaultPriorityCode,DefaultExpirationHours,RequiresHumanReview,SortOrder,IsActive FROM AI.Legal_RecommendationType WHERE (TenantId IS NULL OR TenantId=@TenantId) AND IsDeleted=0 AND IsActive=1 ORDER BY SortOrder,DisplayName;""";using var connection=await connectionFactory.CreateOpenConnectionAsync(cancellationToken);return (await connection.QueryAsync<RecommendationTypeDto>(new CommandDefinition(sql,new{TenantId=tenantId},cancellationToken:cancellationToken))).AsList();
    }
    ...
    private async Task<IntelligenceSearchResponse> SearchPrimaryContactForAccountAsync(IntelligenceSearchRequest request,string effectiveQuery,string? moduleCode,string? entityTypeCode,IReadOnlyCollection<SemanticConceptMatchDto> concepts,Stopwatch timer,CancellationToken cancellationToken)
    {
        const string sql="""
            ...
            (
                ...
                LEFT JOIN AI.Legal_SearchDocument accountDocument ON accountDocument.TenantId=account.TenantId AND accountDocument.EntityTypeCode=N'Account' AND accountDocument.EntityId=account.AccountId AND accountDocument.IsDeleted=0
            ...
            (
                ...
                JOIN AI.Legal_SearchDocument document ON document.TenantId=contact.TenantId AND document.EntityTypeCode=N'Contact' AND document.EntityId=contact.ContactId AND document.IsDeleted=0
                LEFT JOIN Search.EntityProjection projection ON projection.TenantId=document.TenantId AND projection.EntityTypeCode=document.EntityTypeCode AND projection.EntityId=document.EntityId AND projection.IsActive=1 AND projection.IsDeleted=0
                WHERE (@ModuleCode IS NULL OR document.ModuleCode=@ModuleCode) AND (@EntityTypeCode IS NULL OR document.EntityTypeCode=@EntityTypeCode)
            ...
            INSERT AI.Legal_SearchQuery(SearchQueryId,TenantId,UserId,QueryText,NormalizedQuery,SearchModeCode,ExpandedConceptIdsJson,FilterJson,ResultCount,DurationMilliseconds,CorrelationId,CreatedDateUtc,CreatedByUserId,IsDeleted)
    ...
    private async Task<IntelligenceSearchResponse> SearchProducerForAccountAsync(IntelligenceSearchRequest request,string effectiveQuery,string? moduleCode,string? entityTypeCode,IReadOnlyCollection<SemanticConceptMatchDto> concepts,Stopwatch timer,CancellationToken cancellationToken)
    {
        const string sql="""
            ...
            (
                ...
                JOIN AI.Legal_SearchDocument document ON document.TenantId=staff.TenantId AND document.EntityTypeCode=N'Producer' AND document.EntityId=staff.StaffId AND document.IsDeleted=0
                LEFT JOIN Search.EntityProjection projection ON projection.TenantId=document.TenantId AND projection.EntityTypeCode=document.EntityTypeCode AND projection.EntityId=document.EntityId AND projection.IsActive=1 AND projection.IsDeleted=0
                WHERE (@ModuleCode IS NULL OR document.ModuleCode=@ModuleCode) AND (@EntityTypeCode IS NULL OR document.EntityTypeCode=@EntityTypeCode)
