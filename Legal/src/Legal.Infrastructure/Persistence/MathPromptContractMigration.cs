...
// Versioned Math-only data migration. It seeds a structured-output prompt version (PromptCode
// <BASE>_STRUCTURED) for every MATH_* stage, mirroring AstraPromptContractMigration. Existing prompt
// versions and all non-Math rows remain unchanged; re-running is idempotent (guarded by VersionLabel).
public static class MathPromptContractMigration
{
    public const string VersionLabel="v1.0-math-contract";
...
""";
    ...
    public static async Task<int> ApplyAsync(IDbConnection connection,CancellationToken cancellationToken=default)
    {
        ...
        try
        {
            var lockResult=await connection.ExecuteScalarAsync<int>(new CommandDefinition("DECLARE @Result int; EXEC @Result=sys.sp_getapplock @Resource=N'Legal.MathPromptContracts.v1',@LockMode=N'Exclusive',@LockOwner=N'Transaction',@LockTimeout=30000; SELECT @Result;",transaction:transaction,cancellationToken:cancellationToken));
            if(lockResult<0)throw new InvalidOperationException($"Could not acquire Math prompt migration lock: {lockResult}.");
            var inserted=0;
            foreach(var (promptCode,contract) in MathPromptContracts.All)
            {
                ...
                if(schema.RootElement.GetProperty("type").GetString()!="object"||!schema.RootElement.TryGetProperty("properties",out _))
                    throw new InvalidOperationException($"Math output contract is not an object schema: {promptCode}.");
...
WHERE PromptCode=@PromptCode AND TenantId IS NULL AND IsDeleted=0 AND StatusCode=N'APPROVED'
...
IF @CapabilityId IS NULL OR @ApprovedByUserId IS NULL
    THROW 51000,'An effective approved global Math base prompt is required before migrating Math contracts.',1;
WITH scopes AS
(
    SELECT CAST(NULL AS uniqueidentifier) TenantId
    UNION
    SELECT TenantId FROM AI.Legal_PromptDefinition WHERE PromptCode=@StructuredCode AND IsDeleted=0
)
...
SELECT NEWID(),scope.TenantId,COALESCE(source.IntelligenceCapabilityId,@CapabilityId),@StructuredCode,@VersionLabel,@DisplayName,
...
(
    ...
    WHERE prompt.PromptCode IN (@StructuredCode,@PromptCode) AND prompt.IsDeleted=0 AND prompt.StatusCode=N'APPROVED'
    ...
    ORDER BY CASE WHEN prompt.TenantId=scope.TenantId THEN 0 ELSE 1 END,
             CASE WHEN prompt.PromptCode=@StructuredCode THEN 0 ELSE 1 END,prompt.EffectiveFromUtc DESC,prompt.CreatedDateUtc DESC
) source
WHERE NOT EXISTS
(
    SELECT 1 FROM AI.Legal_PromptDefinition existing
    WHERE existing.PromptCode=@StructuredCode AND existing.VersionLabel=@VersionLabel
