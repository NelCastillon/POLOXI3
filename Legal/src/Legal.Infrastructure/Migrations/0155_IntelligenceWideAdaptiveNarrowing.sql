...
-- ── POLOXI.Legal_WideNarrowingIteration: one row per narrowing evaluation (per information round) ──
IF OBJECT_ID(N'POLOXI.Legal_WideNarrowingIteration',N'U') IS NULL
BEGIN
	CREATE TABLE POLOXI.Legal_WideNarrowingIteration
	(
		WideNarrowingIterationId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_PoloxiWideNarrowingIteration PRIMARY KEY,
		WideExecutionId UNIQUEIDENTIFIER NOT NULL CONSTRAINT FK_Legal_PoloxiWideNarrowingIteration_Execution REFERENCES POLOXI.Legal_WideExecution(WideExecutionId),
		...
		ResolvedBranchCount INT NOT NULL CONSTRAINT DF_Legal_PoloxiWideNarrowIter_Resolved DEFAULT 0,
		ReopenedBranchCount INT NOT NULL CONSTRAINT DF_Legal_PoloxiWideNarrowIter_Reopened DEFAULT 0,
		AdmittedCandidateCount INT NOT NULL CONSTRAINT DF_Legal_PoloxiWideNarrowIter_Admitted DEFAULT 0,
		DiscoveredNotAdmittedCount INT NOT NULL CONSTRAINT DF_Legal_PoloxiWideNarrowIter_NotAdmitted DEFAULT 0,
		TransitionsJson NVARCHAR(MAX) NOT NULL CONSTRAINT DF_Legal_PoloxiWideNarrowIter_Transitions DEFAULT N'[]',
		CreatedDateUtc DATETIME2(3) NOT NULL CONSTRAINT DF_Legal_PoloxiWideNarrowIter_Created DEFAULT SYSUTCDATETIME(),
		CreatedByUserId UNIQUEIDENTIFIER NULL,
		ModifiedDateUtc DATETIME2(3) NULL,
		ModifiedByUserId UNIQUEIDENTIFIER NULL,
		IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_PoloxiWideNarrowIter_Deleted DEFAULT 0
	);
	CREATE INDEX IX_Legal_PoloxiWideNarrowIter_Execution ON POLOXI.Legal_WideNarrowingIteration(WideExecutionId,RoundNumber) WHERE IsDeleted=0;
END;
