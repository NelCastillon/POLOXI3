...
BEGIN
	...
	(N'Intelligence.PromptRegistry.ExecutableSource',N'DMS.AiPromptDefinition',N'String',N'Executable prompt source',N'Authoritative executable prompt source while AI.Legal_PromptDefinition provides the cross-capability registry and governance view.');
...
IF OBJECT_ID(N'AI.Legal_PromptDefinition',N'U') IS NOT NULL AND OBJECT_ID(N'DMS.AiPromptDefinition',N'U') IS NOT NULL
BEGIN
	DECLARE @DocumentCapabilityId UNIQUEIDENTIFIER=(SELECT TOP(1) IntelligenceCapabilityId FROM AI.Legal_IntelligenceCapability WHERE CapabilityCode=N'DOCUMENT_INTELLIGENCE' AND TenantId IS NULL AND IsDeleted=0);
	IF @DocumentCapabilityId IS NOT NULL
	BEGIN
		MERGE AI.Legal_PromptDefinition target USING
		(
