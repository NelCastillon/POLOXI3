...
-- ─────────────────────────────────────────────────────────────────────────────────────────────────
-- POLOXI Legal — GENERAL TEST MATTER (M7 / S7).
--
-- A complete, self-contained test matter for /legal/matters and the decision cockpit. Unlike a bare
-- matter row, this seeds the full aggregate the UI reads: the matter (with structured metadata
-- columns used by the list/detail query), a linked decision session (with a persisted Next Best
-- Action), two ranked candidates (a winner + a runner-up), two decision branches (one resolved,
-- one still on the frontier), and verified evidence — so the dashboard card, cockpit verdict, and
-- candidate/branch panels all render real DB-backed state.
--
-- Idempotent: keyed on deterministic GUIDs (A7000000-*) so re-running the migrator never duplicates
-- rows. Tenant-scoped to the development demo tenant (00000000-…-0001) issued by
-- DevelopmentAuthenticationHandler. Runs only after the base decision tables exist.
--
-- NOTE: shipped as a new migration filename (not an edit of 0236) because LegalDatabaseMigrator
-- tracks applied scripts by filename in dbo._LegalMigrations and never re-runs an already-applied
-- name. A new filename guarantees this seed actually executes on the next API startup.
-- ─────────────────────────────────────────────────────────────────────────────────────────────────
...
-- Deterministic ids ── Matter 7 (Acme v. Contoso — summary judgment).
...
DECLARE @S7C1 UNIQUEIDENTIFIER = N'A7000000-0000-0000-0000-000000000021';  -- winner (SJ granted on breach)
DECLARE @S7C2 UNIQUEIDENTIFIER = N'A7000000-0000-0000-0000-000000000022';  -- runner-up (SJ denied on bad-faith)
DECLARE @S7B1 UNIQUEIDENTIFIER = N'A7000000-0000-0000-0000-000000000031';  -- resolved dispositive branch
DECLARE @S7B2 UNIQUEIDENTIFIER = N'A7000000-0000-0000-0000-000000000032';  -- open frontier branch
DECLARE @S7E1 UNIQUEIDENTIFIER = N'A7000000-0000-0000-0000-000000000041';  -- verified evidence
...
BEGIN
	...
	IF NOT EXISTS (SELECT 1 FROM POLOXI.Legal_DecisionMatter WHERE DecisionMatterId = @M7 AND TenantId = @Tenant)
		...
		VALUES
			(@M7,
			 ...
			 N'General end-to-end test matter used to validate the Legal decision cockpit, matter dashboard, candidate ranking, and branch panels on a fresh database. Summary judgment is likely on the breach claim but a genuine dispute of material fact remains on the bad-faith claim.',
			 N'OPEN',
			 N'Contract',
	...
	IF NOT EXISTS (SELECT 1 FROM POLOXI.Legal_DecisionCandidate WHERE DecisionCandidateId = @S7C1)
		...
		VALUES (@S7C1, @S7, N'C1', N'SJ granted on breach claim',
	...
	IF NOT EXISTS (SELECT 1 FROM POLOXI.Legal_DecisionCandidate WHERE DecisionCandidateId = @S7C2)
		INSERT POLOXI.Legal_DecisionCandidate (DecisionCandidateId, DecisionSessionId, CandidateCode, DisplayName, Outcome,
			LegalSupport, FactSupport, EvidenceSupport, AuthoritySupport, VerificationScore, Uncertainty, Discrimination,
			RankingImpact, CompositeScore, RankOrder, IsWinner, TenantId, CreatedDateUtc, CreatedByUserId)
		VALUES (@S7C2, @S7, N'C2', N'SJ denied on bad-faith claim',
	...
	IF NOT EXISTS (SELECT 1 FROM POLOXI.Legal_DecisionBranch WHERE DecisionBranchId = @S7B1)
		...
		VALUES (@S7B1, @S7, 1, N'C1.B1', N'Genuine dispute of material fact on the breach claim?',
	...
	IF NOT EXISTS (SELECT 1 FROM POLOXI.Legal_DecisionBranch WHERE DecisionBranchId = @S7B2)
		INSERT POLOXI.Legal_DecisionBranch (DecisionBranchId, DecisionSessionId, LevelNumber, BranchCode, DisplayName,
			Interpretation, BranchStateCode, InformationValue, DecisionRelevance, FlipPotential, EvidenceAvailability,
			AdvScore, IsOnFrontier, SortOrder, TenantId, CreatedDateUtc, CreatedByUserId)
		VALUES (@S7B2, @S7, 1, N'C2.B1', N'Sufficiency of bad-faith evidence',
	...
	IF NOT EXISTS (SELECT 1 FROM POLOXI.Legal_DecisionEvidence WHERE DecisionEvidenceId = @S7E1)
		...
		VALUES (@S7E1, @S7, @S7B1, N'Executed master services agreement §4 (payment terms) and undisputed non-payment record',
