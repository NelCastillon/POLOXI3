...
BEGIN
	IF COL_LENGTH(N'Submissions.BindRequirement',N'CanBeWaived') IS NULL ALTER TABLE Submissions.BindRequirement ADD CanBeWaived BIT NOT NULL CONSTRAINT DF_Legal_BindRequirement_CanBeWaived_0276 DEFAULT 0;
...
IF OBJECT_ID(N'AI.Legal_ComplianceRequirement',N'U') IS NULL
BEGIN
	CREATE TABLE AI.Legal_ComplianceRequirement
	(
		ComplianceRequirementId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_AI_ComplianceRequirement PRIMARY KEY DEFAULT NEWID(),
		...
		CreatedDateUtc DATETIME2 NOT NULL CONSTRAINT DF_Legal_AI_ComplianceRequirement_Created DEFAULT SYSUTCDATETIME(),
		CreatedByUserId UNIQUEIDENTIFIER NULL,
		ModifiedDateUtc DATETIME2 NULL,
		ModifiedByUserId UNIQUEIDENTIFIER NULL,
		IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_AI_ComplianceRequirement_Deleted DEFAULT 0,
		RowVersion ROWVERSION NOT NULL,
		CONSTRAINT CK_Legal_AI_ComplianceRequirement_Json CHECK(ISJSON(EvaluationJson)=1),
		CONSTRAINT CK_Legal_AI_ComplianceRequirement_Version CHECK(VersionNumber>0),
		CONSTRAINT CK_Legal_AI_ComplianceRequirement_Dates CHECK(EffectiveToUtc IS NULL OR EffectiveToUtc>EffectiveFromUtc)
	);
	CREATE UNIQUE INDEX UX_AI_ComplianceRequirement_Version ON AI.Legal_ComplianceRequirement(TenantId,RequirementCode,VersionNumber) WHERE IsDeleted=0;
	CREATE INDEX IX_Legal_AI_ComplianceRequirement_Scope ON AI.Legal_ComplianceRequirement(TenantId,EntityTypeCode,RequirementScopeCode,JurisdictionCode,CarrierId,LineOfBusinessCode,IsActive) INCLUDE(RequirementCode,SeverityCode,BlocksTransaction,CanBeWaived);
END;

IF OBJECT_ID(N'AI.Legal_EvaluationSampleLabel',N'U') IS NULL
BEGIN
	CREATE TABLE AI.Legal_EvaluationSampleLabel
	(
		EvaluationSampleLabelId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_AI_EvaluationSampleLabel PRIMARY KEY DEFAULT NEWID(),
		TenantId UNIQUEIDENTIFIER NOT NULL,
		...
		CreatedDateUtc DATETIME2 NOT NULL CONSTRAINT DF_Legal_AI_EvaluationSampleLabel_Created DEFAULT SYSUTCDATETIME(),
		CreatedByUserId UNIQUEIDENTIFIER NULL,
		ModifiedDateUtc DATETIME2 NULL,
		ModifiedByUserId UNIQUEIDENTIFIER NULL,
		IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_AI_EvaluationSampleLabel_Deleted DEFAULT 0,
		RowVersion ROWVERSION NOT NULL,
		CONSTRAINT FK_Legal_AI_EvaluationSampleLabel_Execution FOREIGN KEY(ExecutionId) REFERENCES AI.Legal_Execution(ExecutionId),
		CONSTRAINT FK_Legal_AI_EvaluationSampleLabel_Definition FOREIGN KEY(EvaluationDefinitionId) REFERENCES AI.Legal_EvaluationDefinition(EvaluationDefinitionId)
	);
	CREATE INDEX IX_Legal_AI_EvaluationSampleLabel_Window ON AI.Legal_EvaluationSampleLabel(TenantId,LabeledDateUtc,IsDeleted) INCLUDE(PredictedPositive,ActualPositive,IsHallucination,IsAccurate,ExecutionId);
END;

IF OBJECT_ID(N'AI.Legal_SafetyEvent',N'U') IS NULL
BEGIN
	CREATE TABLE AI.Legal_SafetyEvent
	(
		SafetyEventId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_AI_SafetyEvent PRIMARY KEY DEFAULT NEWID(),
		TenantId UNIQUEIDENTIFIER NOT NULL,
		...
		CreatedDateUtc DATETIME2 NOT NULL CONSTRAINT DF_Legal_AI_SafetyEvent_Created DEFAULT SYSUTCDATETIME(),
		CreatedByUserId UNIQUEIDENTIFIER NULL,
		ModifiedDateUtc DATETIME2 NULL,
		ModifiedByUserId UNIQUEIDENTIFIER NULL,
		IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_AI_SafetyEvent_Deleted DEFAULT 0,
		RowVersion ROWVERSION NOT NULL,
		CONSTRAINT FK_Legal_AI_SafetyEvent_Control FOREIGN KEY(SafetyControlId) REFERENCES AI.Legal_SafetyControl(SafetyControlId),
		CONSTRAINT FK_Legal_AI_SafetyEvent_Execution FOREIGN KEY(ExecutionId) REFERENCES AI.Legal_Execution(ExecutionId),
		CONSTRAINT FK_Legal_AI_SafetyEvent_Reasoning FOREIGN KEY(ReasoningSessionId) REFERENCES AI.Legal_ReasoningSession(ReasoningSessionId),
		CONSTRAINT CK_Legal_AI_SafetyEvent_Json CHECK(ISJSON(DetailsJson)=1)
	);
	CREATE INDEX IX_Legal_AI_SafetyEvent_TenantDate ON AI.Legal_SafetyEvent(TenantId,DetectedDateUtc DESC) INCLUDE(SafetyControlId,EventTypeCode,ActionCode,SeverityCode,RequiresHumanReview,ReviewStatusCode);
END;

MERGE AI.Legal_EvaluationDefinition target USING(VALUES
