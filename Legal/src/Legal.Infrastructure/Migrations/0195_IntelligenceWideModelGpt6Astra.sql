...
-- Wide-search model selection: seed the gpt-6-astra CHAT deployment under the shared
-- Azure OpenAI provider so the /intelligence/search/poloxi_wide model dropdown is fully
-- database-backed (Auto = feature-policy routing; explicit models come from AI.Legal_ModelDeployment).
-- Mirrors 0161_IntelligenceWideModelSelection.sql (gpt-5.6-sol); the Azure endpoint/credential
-- are shared per provider, so only the model deployment row is required.
...
BEGIN
	...
	(
		...
		WHERE ProviderId=@ProviderId AND ModelCode=N'gpt-6-astra' AND IsDeleted=0
	)
	BEGIN
		...
		VALUES
		(
			NEWID(),NULL,@ProviderId,N'gpt-6-astra',N'gpt-6-astra',N'GPT-6',N'CHAT',
