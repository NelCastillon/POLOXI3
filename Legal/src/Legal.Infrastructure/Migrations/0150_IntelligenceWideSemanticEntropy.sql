...
-- ── POLOXI.Legal_WideBranch: semantic type (ALTERNATIVE | DIMENSION) ──
IF OBJECT_ID(N'POLOXI.Legal_WideBranch',N'U') IS NOT NULL
BEGIN
	IF COL_LENGTH(N'POLOXI.Legal_WideBranch',N'SemanticTypeCode') IS NULL
		ALTER TABLE POLOXI.Legal_WideBranch ADD SemanticTypeCode NVARCHAR(20) NOT NULL CONSTRAINT DF_Legal_PoloxiWideBranch_SemanticType DEFAULT N'ALTERNATIVE';
END;

-- ── POLOXI.Legal_WideInformationRound: which distribution entropy was measured over ──
IF OBJECT_ID(N'POLOXI.Legal_WideInformationRound',N'U') IS NOT NULL
BEGIN
	IF COL_LENGTH(N'POLOXI.Legal_WideInformationRound',N'EntropyBasisCode') IS NULL
		ALTER TABLE POLOXI.Legal_WideInformationRound ADD EntropyBasisCode NVARCHAR(20) NOT NULL CONSTRAINT DF_Legal_PoloxiWideInfoRound_EntropyBasis DEFAULT N'BRANCH';
END;

-- ── POLOXI.Legal_WideExecution: execution-level entropy basis ──
IF OBJECT_ID(N'POLOXI.Legal_WideExecution',N'U') IS NOT NULL
BEGIN
	IF COL_LENGTH(N'POLOXI.Legal_WideExecution',N'EntropyBasisCode') IS NULL
		ALTER TABLE POLOXI.Legal_WideExecution ADD EntropyBasisCode NVARCHAR(20) NULL;
END;
