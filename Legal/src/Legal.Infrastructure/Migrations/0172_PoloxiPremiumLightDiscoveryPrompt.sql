...
IF NOT EXISTS(SELECT 1 FROM POLOXI.Legal_PromptStrategy WHERE TenantId IS NULL AND PurposeCode=N'AMBIGUITY_DISCOVERY' AND ScaffoldingCode=N'LIGHT' AND VersionNumber=2 AND IsDeleted=0)
BEGIN
...
IF NOT EXISTS(SELECT 1 FROM POLOXI.Legal_ModelCapabilityProfile WHERE TenantId IS NULL AND ModelCodePattern=N'%sol%' AND IsDeleted=0)
