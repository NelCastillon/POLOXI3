...
IF OBJECT_ID(N'AI.Legal_SafetyEvent',N'U') IS NOT NULL
BEGIN
	IF COL_LENGTH(N'AI.Legal_SafetyEvent',N'IdempotencyKey') IS NULL
		ALTER TABLE AI.Legal_SafetyEvent ADD IdempotencyKey NVARCHAR(240) NULL;

	IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'AI.Legal_SafetyEvent') AND name=N'UX_AI_SafetyEvent_Idempotency')
		EXEC sys.sp_executesql N'CREATE UNIQUE INDEX UX_AI_SafetyEvent_Idempotency ON AI.Legal_SafetyEvent(TenantId,IdempotencyKey) WHERE IsDeleted=0 AND IdempotencyKey IS NOT NULL;';
END;

IF OBJECT_ID(N'AI.Legal_PromptDefinition',N'U') IS NOT NULL
AND NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'AI.Legal_PromptDefinition') AND name=N'UX_AI_PromptDefinition_TenantVersion')
	CREATE UNIQUE INDEX UX_AI_PromptDefinition_TenantVersion ON AI.Legal_PromptDefinition(TenantId,PromptCode,VersionLabel) WHERE TenantId IS NOT NULL AND IsDeleted=0;
