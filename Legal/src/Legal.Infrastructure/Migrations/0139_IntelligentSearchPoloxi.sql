...
IF OBJECT_ID(N'POLOXI.Legal_Capability',N'U') IS NULL
CREATE TABLE POLOXI.Legal_Capability
(
	CapabilityId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_POLOXI_Capability PRIMARY KEY DEFAULT NEWID(),
	...
	IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_POLOXI_Capability_IsDeleted DEFAULT 0,
	RowVersion ROWVERSION NOT NULL,
	CONSTRAINT CK_Legal_POLOXI_Capability_Confidence CHECK(MinimumConfidence BETWEEN 0 AND 1),
	CONSTRAINT CK_Legal_POLOXI_Capability_TermsJson CHECK(ISJSON(ApprovedTermsJson)=1)
);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'POLOXI.Legal_Capability') AND name=N'UX_POLOXI_Capability_Code') CREATE UNIQUE INDEX UX_POLOXI_Capability_Code ON POLOXI.Legal_Capability(CapabilityCode) WHERE TenantId IS NULL AND IsDeleted=0;

IF OBJECT_ID(N'POLOXI.Legal_Hierarchy',N'U') IS NULL
CREATE TABLE POLOXI.Legal_Hierarchy
(
	HierarchyId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_POLOXI_Hierarchy PRIMARY KEY DEFAULT NEWID(),
	...
	UsageCount INT NOT NULL CONSTRAINT DF_Legal_POLOXI_Hierarchy_Usage DEFAULT 0,
	SuccessfulUsageCount INT NOT NULL CONSTRAINT DF_Legal_POLOXI_Hierarchy_Success DEFAULT 0,
	...
	ModifiedDateUtc DATETIME2 NULL,
	ModifiedByUserId UNIQUEIDENTIFIER NULL,
	IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_POLOXI_Hierarchy_IsDeleted DEFAULT 0,
	RowVersion ROWVERSION NOT NULL,
	CONSTRAINT CK_Legal_POLOXI_Hierarchy_Confidence CHECK(Confidence BETWEEN 0 AND 1),
	CONSTRAINT CK_Legal_POLOXI_Hierarchy_Version CHECK(VersionNumber>0)
);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'POLOXI.Legal_Hierarchy') AND name=N'UX_POLOXI_Hierarchy_Version') CREATE UNIQUE INDEX UX_POLOXI_Hierarchy_Version ON POLOXI.Legal_Hierarchy(TenantId,QuerySignature,VersionNumber) WHERE IsDeleted=0;
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'POLOXI.Legal_Hierarchy') AND name=N'IX_Legal_POLOXI_Hierarchy_Reusable') CREATE INDEX IX_Legal_POLOXI_Hierarchy_Reusable ON POLOXI.Legal_Hierarchy(TenantId,QuerySignature,StatusCode,ExpiresDateUtc) INCLUDE(Confidence,UsageCount,SuccessfulUsageCount);

IF OBJECT_ID(N'POLOXI.Legal_HierarchyBranch',N'U') IS NULL
CREATE TABLE POLOXI.Legal_HierarchyBranch
(
	HierarchyBranchId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_POLOXI_HierarchyBranch PRIMARY KEY DEFAULT NEWID(),
	TenantId UNIQUEIDENTIFIER NOT NULL,
	...
	ModifiedDateUtc DATETIME2 NULL,
	ModifiedByUserId UNIQUEIDENTIFIER NULL,
	IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_POLOXI_Branch_IsDeleted DEFAULT 0,
	RowVersion ROWVERSION NOT NULL,
	CONSTRAINT FK_Legal_POLOXI_Branch_Hierarchy FOREIGN KEY(HierarchyId) REFERENCES POLOXI.Legal_Hierarchy(HierarchyId),
	CONSTRAINT FK_Legal_POLOXI_Branch_Parent FOREIGN KEY(ParentHierarchyBranchId) REFERENCES POLOXI.Legal_HierarchyBranch(HierarchyBranchId),
	CONSTRAINT CK_Legal_POLOXI_Branch_Confidence CHECK(Confidence BETWEEN 0 AND 1)
);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'POLOXI.Legal_HierarchyBranch') AND name=N'IX_Legal_POLOXI_Branch_Hierarchy') CREATE INDEX IX_Legal_POLOXI_Branch_Hierarchy ON POLOXI.Legal_HierarchyBranch(TenantId,HierarchyId,SortOrder) INCLUDE(ValidationStatusCode,CapabilityCode);

IF OBJECT_ID(N'POLOXI.Legal_Execution',N'U') IS NULL
CREATE TABLE POLOXI.Legal_Execution
(
	PoloxiExecutionId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_POLOXI_Execution PRIMARY KEY DEFAULT NEWID(),
	TenantId UNIQUEIDENTIFIER NOT NULL,
	HierarchyId UNIQUEIDENTIFIER NOT NULL,
	...
	ModifiedDateUtc DATETIME2 NULL,
	ModifiedByUserId UNIQUEIDENTIFIER NULL,
	IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_POLOXI_Execution_IsDeleted DEFAULT 0,
	RowVersion ROWVERSION NOT NULL,
	CONSTRAINT FK_Legal_POLOXI_Execution_Hierarchy FOREIGN KEY(HierarchyId) REFERENCES POLOXI.Legal_Hierarchy(HierarchyId),
	CONSTRAINT CK_Legal_POLOXI_Execution_Confidence CHECK(Confidence BETWEEN 0 AND 1)
);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'POLOXI.Legal_Execution') AND name=N'IX_Legal_POLOXI_Execution_TenantDate') CREATE INDEX IX_Legal_POLOXI_Execution_TenantDate ON POLOXI.Legal_Execution(TenantId,CreatedDateUtc DESC) INCLUDE(StatusCode,ResultCount,Confidence);

IF OBJECT_ID(N'POLOXI.Legal_ExecutionEvidence',N'U') IS NULL
CREATE TABLE POLOXI.Legal_ExecutionEvidence
(
	ExecutionEvidenceId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_POLOXI_ExecutionEvidence PRIMARY KEY DEFAULT NEWID(),
	TenantId UNIQUEIDENTIFIER NOT NULL,
	...
	ModifiedDateUtc DATETIME2 NULL,
	ModifiedByUserId UNIQUEIDENTIFIER NULL,
	IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_POLOXI_Evidence_IsDeleted DEFAULT 0,
	CONSTRAINT FK_Legal_POLOXI_Evidence_Execution FOREIGN KEY(PoloxiExecutionId) REFERENCES POLOXI.Legal_Execution(PoloxiExecutionId),
	CONSTRAINT FK_Legal_POLOXI_Evidence_Branch FOREIGN KEY(HierarchyBranchId) REFERENCES POLOXI.Legal_HierarchyBranch(HierarchyBranchId),
	CONSTRAINT CK_Legal_POLOXI_Evidence_Score CHECK(RelevanceScore BETWEEN 0 AND 1)
);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'POLOXI.Legal_ExecutionEvidence') AND name=N'IX_Legal_POLOXI_Evidence_Execution') CREATE INDEX IX_Legal_POLOXI_Evidence_Execution ON POLOXI.Legal_ExecutionEvidence(TenantId,PoloxiExecutionId,RankNumber) INCLUDE(HierarchyBranchId,SearchDocumentId,RelevanceScore);
...
IF OBJECT_ID(N'AI.Legal_FeaturePolicy',N'U') IS NOT NULL AND OBJECT_ID(N'AI.Legal_ModelDeployment',N'U') IS NOT NULL AND OBJECT_ID(N'Core.Tenant',N'U') IS NOT NULL
BEGIN
	;WITH Routes AS(SELECT tenant.TenantId,(SELECT TOP(1) model.ModelDeploymentId FROM AI.Legal_ModelDeployment model JOIN AI.Legal_Provider provider ON provider.ProviderId=model.ProviderId AND provider.IsActive=1 AND provider.IsDeleted=0 WHERE model.IsActive=1 AND model.IsDeleted=0 AND model.CapabilityCode=N'CHAT' AND (model.TenantId=tenant.TenantId OR model.TenantId IS NULL) ORDER BY CASE WHEN model.TenantId=tenant.TenantId THEN 0 ELSE 1 END,model.IsFallback,model.Priority) ModelDeploymentId FROM Core.Tenant tenant WHERE tenant.IsDeleted=0),
	...
	MERGE AI.Legal_FeaturePolicy target USING SourcePolicy source ON target.TenantId=source.TenantId AND target.FeatureCode=source.FeatureCode AND target.IsDeleted=0
