...
IF OBJECT_ID(N'POLOXI.Legal_WideExecution',N'U') IS NULL
BEGIN
	CREATE TABLE POLOXI.Legal_WideExecution
	(
		WideExecutionId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_PoloxiWideExecution PRIMARY KEY,
		TenantId UNIQUEIDENTIFIER NOT NULL,
		UserId UNIQUEIDENTIFIER NOT NULL,
		QueryText NVARCHAR(1000) NOT NULL,
		CorrelationId NVARCHAR(120) NOT NULL,
		StatusCode NVARCHAR(50) NOT NULL CONSTRAINT DF_Legal_PoloxiWideExecution_Status DEFAULT N'RUNNING',
		TerminationReasonCode NVARCHAR(50) NULL,
		DepthReached INT NOT NULL CONSTRAINT DF_Legal_PoloxiWideExecution_Depth DEFAULT 0,
		LlmCallCount INT NOT NULL CONSTRAINT DF_Legal_PoloxiWideExecution_Calls DEFAULT 0,
		...
		CreatedDateUtc DATETIME2(3) NOT NULL CONSTRAINT DF_Legal_PoloxiWideExecution_Created DEFAULT SYSUTCDATETIME(),
		CreatedByUserId UNIQUEIDENTIFIER NULL,
		ModifiedDateUtc DATETIME2(3) NULL,
		ModifiedByUserId UNIQUEIDENTIFIER NULL,
		IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_PoloxiWideExecution_Deleted DEFAULT 0
	);
	CREATE INDEX IX_Legal_PoloxiWideExecution_Tenant ON POLOXI.Legal_WideExecution(TenantId,CreatedDateUtc DESC) WHERE IsDeleted=0;
END;

-- Wide branch audit: every proposed branch per level, including eliminated ones (never deleted).
IF OBJECT_ID(N'POLOXI.Legal_WideBranch',N'U') IS NULL
BEGIN
	CREATE TABLE POLOXI.Legal_WideBranch
	(
		WideBranchId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_PoloxiWideBranch PRIMARY KEY,
		WideExecutionId UNIQUEIDENTIFIER NOT NULL CONSTRAINT FK_Legal_PoloxiWideBranch_Execution REFERENCES POLOXI.Legal_WideExecution(WideExecutionId),
		ParentWideBranchId UNIQUEIDENTIFIER NULL CONSTRAINT FK_Legal_PoloxiWideBranch_Parent REFERENCES POLOXI.Legal_WideBranch(WideBranchId),
		TenantId UNIQUEIDENTIFIER NOT NULL,
		...
		EvidenceCount INT NOT NULL CONSTRAINT DF_Legal_PoloxiWideBranch_Evidence DEFAULT 0,
		Confidence DECIMAL(5,4) NOT NULL,
		ContinueNarrowing BIT NOT NULL CONSTRAINT DF_Legal_PoloxiWideBranch_Continue DEFAULT 0,
		StopReason NVARCHAR(50) NULL,
		IsEliminated BIT NOT NULL CONSTRAINT DF_Legal_PoloxiWideBranch_Eliminated DEFAULT 0,
		EliminationReason NVARCHAR(400) NULL,
		SortOrder INT NOT NULL,
		CreatedDateUtc DATETIME2(3) NOT NULL CONSTRAINT DF_Legal_PoloxiWideBranch_Created DEFAULT SYSUTCDATETIME(),
		CreatedByUserId UNIQUEIDENTIFIER NULL,
		ModifiedDateUtc DATETIME2(3) NULL,
		ModifiedByUserId UNIQUEIDENTIFIER NULL,
		IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_PoloxiWideBranch_Deleted DEFAULT 0
	);
	CREATE INDEX IX_Legal_PoloxiWideBranch_Execution ON POLOXI.Legal_WideBranch(WideExecutionId,LevelNumber,SortOrder) WHERE IsDeleted=0;
END;
