...
-- POLOXI.Legal_AnswerKind replaces the C# answer-kind constants and the five per-kind budget
...
IF OBJECT_ID(N'POLOXI.Legal_AnswerKind',N'U') IS NULL
BEGIN
	CREATE TABLE POLOXI.Legal_AnswerKind(
		AnswerKindId UNIQUEIDENTIFIER NOT NULL CONSTRAINT DF_Legal_POLOXI_AnswerKind_Id DEFAULT NEWSEQUENTIALID(),
		TenantId UNIQUEIDENTIFIER NULL,
		AnswerKindCode NVARCHAR(30) NOT NULL,
		DisplayName NVARCHAR(100) NOT NULL,
		Description NVARCHAR(500) NULL,
		DepthCeiling INT NOT NULL CONSTRAINT DF_Legal_POLOXI_AnswerKind_Depth DEFAULT 0,
		MaxInformationRounds INT NULL,
		RunsCandidateCompetition BIT NOT NULL CONSTRAINT DF_Legal_POLOXI_AnswerKind_Competition DEFAULT 1,
		SortOrder INT NOT NULL CONSTRAINT DF_Legal_POLOXI_AnswerKind_Sort DEFAULT 0,
		CreatedDateUtc DATETIME2(3) NOT NULL CONSTRAINT DF_Legal_POLOXI_AnswerKind_Created DEFAULT SYSUTCDATETIME(),
		CreatedByUserId UNIQUEIDENTIFIER NULL,
		ModifiedDateUtc DATETIME2(3) NULL,
		ModifiedByUserId UNIQUEIDENTIFIER NULL,
		IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_POLOXI_AnswerKind_Deleted DEFAULT 0,
		CONSTRAINT PK_Legal_POLOXI_AnswerKind PRIMARY KEY CLUSTERED(AnswerKindId)
	);
	CREATE UNIQUE INDEX UX_POLOXI_AnswerKind_Tenant_Code ON POLOXI.Legal_AnswerKind(AnswerKindCode,TenantId) WHERE IsDeleted=0;
END;

-- Seed the three V3.1/V3.2 kinds as global defaults (idempotent).
MERGE POLOXI.Legal_AnswerKind AS target
USING(VALUES
