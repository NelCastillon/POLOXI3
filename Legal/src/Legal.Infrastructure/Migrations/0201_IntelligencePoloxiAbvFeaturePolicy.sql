...
-- Corrective seed: INTELLIGENCE_POLOXI_ABV (AbvResolutionEngine) was introduced without a matching
-- AI.Legal_FeaturePolicy row, so the router fails every ABV run with
-- "No active CHAT model route is configured for this tenant and feature."
-- This migration idempotently seeds the ABV feature policy for every tenant against the resolved
-- active CHAT route, mirroring the 0180 backfill pattern.
...
BEGIN
	...
	Features AS
	(
		SELECT N'INTELLIGENCE_POLOXI_ABV' FeatureCode,CONVERT(decimal(4,3),0.000) Temperature,14000 MaximumInputTokens,4000 MaximumOutputTokens,60 TimeoutSeconds,CONVERT(decimal(5,4),0.0000) MinimumConfidence
	),
	SourcePolicy AS
	(
