...
IF COL_LENGTH(N'AI.Legal_SearchQuery',N'ScoringWeightsJson') IS NULL ALTER TABLE AI.Legal_SearchQuery ADD ScoringWeightsJson NVARCHAR(MAX) NULL;
IF COL_LENGTH(N'AI.Legal_SearchQuery',N'SummaryStatusCode') IS NULL ALTER TABLE AI.Legal_SearchQuery ADD SummaryStatusCode NVARCHAR(30) NULL;
IF COL_LENGTH(N'AI.Legal_SearchQuery',N'SummaryExecutionId') IS NULL ALTER TABLE AI.Legal_SearchQuery ADD SummaryExecutionId UNIQUEIDENTIFIER NULL;
IF NOT EXISTS(SELECT 1 FROM sys.check_constraints WHERE name=N'CK_Legal_AI_SearchQuery_ScoringWeightsJson') EXEC(N'ALTER TABLE AI.Legal_SearchQuery ADD CONSTRAINT CK_Legal_AI_SearchQuery_ScoringWeightsJson CHECK(ScoringWeightsJson IS NULL OR ISJSON(ScoringWeightsJson)=1);');

IF OBJECT_ID(N'AI.Legal_SearchResultEvidence',N'U') IS NULL
BEGIN
	CREATE TABLE AI.Legal_SearchResultEvidence
	(
		SearchResultEvidenceId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_AI_SearchResultEvidence PRIMARY KEY DEFAULT NEWID(),
		...
		CreatedDateUtc DATETIME2 NOT NULL CONSTRAINT DF_Legal_AI_SearchResultEvidence_Created DEFAULT SYSUTCDATETIME(),
		CreatedByUserId UNIQUEIDENTIFIER NULL,
		IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_AI_SearchResultEvidence_Deleted DEFAULT 0,
		CONSTRAINT FK_Legal_AI_SearchResultEvidence_Query FOREIGN KEY(SearchQueryId) REFERENCES AI.Legal_SearchQuery(SearchQueryId),
		CONSTRAINT FK_Legal_AI_SearchResultEvidence_Document FOREIGN KEY(SearchDocumentId) REFERENCES AI.Legal_SearchDocument(SearchDocumentId),
		CONSTRAINT CK_Legal_AI_SearchResultEvidence_Explanations CHECK(ISJSON(ExplanationsJson)=1)
	);
	CREATE INDEX IX_Legal_AI_SearchResultEvidence_Query ON AI.Legal_SearchResultEvidence(TenantId,SearchQueryId,RankNumber) INCLUDE(SearchDocumentId,UnifiedScore);
END;
