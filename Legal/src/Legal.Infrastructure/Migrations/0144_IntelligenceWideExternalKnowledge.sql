...
IF OBJECT_ID(N'POLOXI.Legal_ExternalKnowledge',N'U') IS NULL
BEGIN
	CREATE TABLE POLOXI.Legal_ExternalKnowledge
	(
		ExternalKnowledgeId UNIQUEIDENTIFIER NOT NULL CONSTRAINT PK_Legal_PoloxiExternalKnowledge PRIMARY KEY,
		TenantId UNIQUEIDENTIFIER NOT NULL,
		NormalizedQuery NVARCHAR(400) NOT NULL,
		Title NVARCHAR(500) NOT NULL,
		Url NVARCHAR(2000) NOT NULL,
		Snippet NVARCHAR(MAX) NOT NULL,
		Score DECIMAL(5,4) NOT NULL CONSTRAINT DF_Legal_PoloxiExternalKnowledge_Score DEFAULT 0,
		RetrievedDateUtc DATETIME2(3) NOT NULL,
		CreatedDateUtc DATETIME2(3) NOT NULL CONSTRAINT DF_Legal_PoloxiExternalKnowledge_Created DEFAULT SYSUTCDATETIME(),
		CreatedByUserId UNIQUEIDENTIFIER NULL,
		ModifiedDateUtc DATETIME2(3) NULL,
		ModifiedByUserId UNIQUEIDENTIFIER NULL,
		IsDeleted BIT NOT NULL CONSTRAINT DF_Legal_PoloxiExternalKnowledge_Deleted DEFAULT 0
	);
	CREATE INDEX IX_Legal_PoloxiExternalKnowledge_TenantQuery ON POLOXI.Legal_ExternalKnowledge(TenantId,NormalizedQuery,RetrievedDateUtc DESC) WHERE IsDeleted=0;
END;
