...
public sealed class IntelligenceWideService(IIntelligenceRepository repository,IIntelligenceWideRepository wideRepository,IAiProviderRouter aiProviderRouter,IExternalKnowledgeProvider externalKnowledgeProvider,ILegalRetriever legalRetriever,IPromptCatalog promptCatalog,IAdaptiveRetriever adaptiveRetriever,IAbvResolutionEngine abvEngine,ILogger<IntelligenceWideService> logger):IIntelligenceWideService
{
    ...
    public Task<IReadOnlyCollection<LegalGroundingSettingDto>> GetLegalGroundingSettingsAsync(CancellationToken cancellationToken=default)=>wideRepository.GetLegalGroundingSettingsAsync(cancellationToken);

    public Task SaveLegalGroundingSettingAsync(SaveLegalGroundingSettingRequest request,Guid actorUserId,CancellationToken cancellationToken=default)=>wideRepository.SaveLegalGroundingSettingAsync(request,actorUserId,cancellationToken);
...
""";
    ...
    // Defect C (legal grounding fallback): when authority extraction finds no named case/statute/CFR
    // citation in a branch, the generic concept query (whole user question + branch name) is too
    // diluted to return verifiable authorities. This builds a DOCTRINAL query instead: it draws the
    // decisive doctrinal terms from the branch's own interpretation (DisplayName + Interpretation +
    // SearchText) and pairs them with the distinctive legal terms of the user's question, so the
    // missing doctrinal information is routed into targeted CourtListener/GovInfo searches. Isolated to
    // the legal retrieval layer; deterministic; falls back to the concept query when nothing distinctive
    // remains.
    private static string BuildDoctrinalFallbackQuery(string query,WideBranchRecord branch)
    {
        var seen=new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        var doctrinal=new List<string>();
        void Harvest(string? source,int limit)
        {
            ...
            foreach(var raw in source.Split([' ','\t','\n','\r',',',';','/','(',')',':','.','?','!','"','\''],StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                if(!seen.Add(token))continue;
                doctrinal.Add(token);
                if(doctrinal.Count>=limit)break;
            }
        }
        // Branch doctrine first (the decisive interpretation this branch represents), then the user's
        // distinctive legal terms, so retrieval leads with doctrine rather than generic phrasing.
        Harvest(branch.DisplayName,6);
        Harvest(branch.Interpretation,10);
        Harvest(branch.SearchText,12);
        Harvest(query,16);
        if(doctrinal.Count==0)return BuildCandidateSeekingQuery(query,branch.DisplayName);
    ...
    private static WideExternalKnowledgeSnippet StampBranch(WideExternalKnowledgeSnippet snippet,WideBranchRecord branch,string attributionQuery)=>
        snippet with{Query=attributionQuery,BranchId=branch.WideBranchId,BranchDisplayName=branch.DisplayName};
    ...
    public async Task<WideSearchResponse> SearchDynamicAsync(WideSearchRequest request,CancellationToken cancellationToken=default)
    {
        ...
        var timer=Stopwatch.StartNew();
        request=request with{Query=NormalizeQuery(request.Query),MaximumResults=Math.Clamp(request.MaximumResults,1,100),CorrelationId=string.IsNullOrWhiteSpace(request.CorrelationId)?$"wide-search:{Guid.NewGuid():N}":request.CorrelationId.Trim(),ContextCode=string.IsNullOrWhiteSpace(request.ContextCode)?WideSearchContexts.General:request.ContextCode.Trim().ToUpperInvariant()};
        ...
        try
        {
            ...
            // has decided; one fail-soft LLM call renders the decision as senior-attorney prose, gated by a
            // deterministic output guard. Never on the clarification gate. Any failure/guard rejection
            // leaves LegalAnswer null and the standard POLOXI answer stands — POLOXI Core is untouched.
            WideLegalAnswerDto? legalAnswer=null;
            if(string.Equals(request.ContextCode?.Trim(),WideSearchContexts.Legal,StringComparison.OrdinalIgnoreCase)&&answerStatus!="USER_CLARIFICATION_REQUIRED"&&!string.IsNullOrWhiteSpace(finalAnswerText))
            {
                llmCalls++;
    ...
    // Legal Answer Composer output schema. Presentation-only structure; every field is prose the UI
    // renders directly. keyAuthorities entries must echo the supplied verified authorities.
    private const string LegalAnswerSchema="""
{
  "type": "object",
  "properties": {
    ...
    "decisiveMissingFact": { "type": ["string", "null"] },
    "bottomLine": { "type": "string" }
  },
  "required": ["conclusion", "analysis", "keyAuthorities", "application", "bottomLine"],
  "additionalProperties": false
}
""";

    // LLM proposal shape for the Legal Answer Composer (deserialized from LegalAnswerSchema).
    private sealed record LegalAnswerAuthorityProposal(string? Name,string? Relevance,string? Reference);
    private sealed record LegalAnswerProposal(string? Conclusion,string? Analysis,IReadOnlyList<LegalAnswerAuthorityProposal>? KeyAuthorities,string? Application,string? Counterargument,string? RemainingUncertainty,string? DecisiveMissingFact,string? BottomLine);

    // Builds the deterministic user prompt for the Legal Answer Composer from state POLOXI already
    // computed. It carries the POLOXI conclusion (the answer text/deliverable), the winning
    // interpretation, the residual uncertainty, the VERIFIED authorities, and the already-produced
    ...
    private static string BuildLegalComposerContext(WideSearchRequest request,string poloxiConclusion,string? winnerDisplayName,WideResolutionDeliverableDto? resolutionDeliverable,IReadOnlyList<WideExternalKnowledgeSnippet> verifiedAuthorities,IReadOnlyCollection<WideInterpretiveResultDto> interpretiveResults,string interpretationStatus,string evidenceStatus,decimal? decisionConfidence,decimal decisionEvidenceCoverage,decimal evidenceCoverage,WideEntropyResult finalEntropy)
    {
        ...
        if(decisionConfidence is not null)
            builder.Append("INTERPRETATION CONFIDENCE: ").Append(decisionConfidence.Value.ToString("P0")).Append('\n');
        builder.Append("EVIDENCE STATUS: ").Append(evidenceStatus).Append('\n');
        builder.Append("EVIDENCE COVERAGE: ").Append(evidenceCoverage.ToString("P0")).Append(" | DECISION EVIDENCE COVERAGE: ").Append(decisionEvidenceCoverage.ToString("P0")).Append('\n');
        if(resolutionDeliverable is not null)
        {
            builder.Append("POLOXI DETERMINACY: ").Append(resolutionDeliverable.DeterminacyCode).Append('\n');
            if(!string.IsNullOrWhiteSpace(resolutionDeliverable.Outcome))
                builder.Append("POLOXI OUTCOME: ").Append(Truncate(resolutionDeliverable.Outcome,1500)).Append('\n');
        }
        builder.Append("POLOXI CONCLUSION (do NOT change the substance):\n").Append(Truncate(poloxiConclusion,4000)).Append('\n');
        builder.Append("RESIDUAL UNCERTAINTY: ").Append(finalEntropy.NormalizedEntropy.ToString("P0")).Append('\n');
        // Substantive POLOXI interpretation results: these are the already-resolved meanings POLOXI
        // produced. They are model-derived (not independently verified) unless flagged externally grounded.
        if(interpretiveResults.Count>0)
        {
            builder.Append("POLOXI INTERPRETATION RESULTS (substantive input — present these, but label model-derived ones as not yet independently verified):\n");
            foreach(var result in interpretiveResults.Take(8))
            {
                builder.Append("- [").Append(result.IsExternallyGrounded?"VERIFIED":"MODEL-DERIVED").Append("] ")
                    .Append(Truncate(result.BranchDisplayName,200)).Append(": ").Append(Truncate(result.Interpretation,600)).Append('\n');
                foreach(var item in result.Items.Take(4))
                    builder.Append("    • ").Append(Truncate(item.Name,160)).Append(" — ").Append(Truncate(item.Detail,400)).Append('\n');
            }
        }
        if(verifiedAuthorities.Count==0)
        {
            builder.Append("VERIFIED AUTHORITIES: (none independently verified — return an empty keyAuthorities array and do NOT cite any authority as verified; you MAY still explain the POLOXI interpretation, clearly stating the supporting authority has not yet been independently verified)\n");
        }
        else
        {
            builder.Append("VERIFIED AUTHORITIES (cite ONLY these, echo the name verbatim):\n");
            foreach(var authority in verifiedAuthorities.Take(8))
                builder.Append("- ").Append(Truncate(authority.Title,200)).Append(" (").Append(authority.Url).Append("): ").Append(Truncate(authority.Snippet,400)).Append('\n');
        }
        return builder.ToString();
    }

    // Legal Answer Composer (PRESENTATION ONLY, fail-soft — mirrors the ABV stage). One fast LLM call
    // rewrites the decision POLOXI already made into senior-attorney prose. It never re-runs reasoning,
    // never changes the conclusion, and its result is discarded entirely if the deterministic output
    // guard finds an invented authority or a contradicted conclusion. Any failure returns null and the
    // standard POLOXI answer stands. Never touches ranking, confidence, answer status, or persistence.
    private async Task<WideLegalAnswerDto?> ComposeLegalAnswerAsync(WideSearchRequest request,string poloxiConclusion,string? winnerDisplayName,WideResolutionDeliverableDto? resolutionDeliverable,IReadOnlyCollection<WideExternalKnowledgeSnippet> externalKnowledge,IReadOnlyCollection<WideInterpretiveResultDto> interpretiveResults,decimal? decisionConfidence,decimal decisionEvidenceCoverage,decimal evidenceCoverage,WideEntropyResult finalEntropy,CancellationToken cancellationToken)
    {
        try
        {
            var verifiedAuthorities=externalKnowledge
                .Where(snippet=>snippet.AuthorityIdentityVerified&&!string.IsNullOrWhiteSpace(snippet.Title))
                .OrderByDescending(snippet=>snippet.PropositionSupportScore)
                .ThenByDescending(snippet=>snippet.Score)
                .ToArray();
            // Interpretation status is independent from evidence status. POLOXI can RESOLVE a legal
            // interpretation from model knowledge even when zero authorities were independently verified;
            // the attorney answer must present that interpretation (labeled unverified), not refuse it.
            var determinacy=resolutionDeliverable?.DeterminacyCode;
            var interpretationStatus=string.Equals(determinacy,"BLOCKED",StringComparison.OrdinalIgnoreCase)?"UNRESOLVED"
                :interpretiveResults.Count==0?"UNRESOLVED"
                :string.Equals(determinacy,"PARTIAL",StringComparison.OrdinalIgnoreCase)?"PARTIALLY_RESOLVED"
                :"RESOLVED";
            var evidenceStatus=verifiedAuthorities.Length==0?"UNVERIFIED"
        ...
        {
            // Presentation only: never surface composer failures to the user-facing answer.
            return null;
        }
    }
    ...
    // (no invented authorities); (3) the composed conclusion must not contradict the POLOXI winner or a
    // BLOCKED determinacy. Any violation discards the WHOLE composed answer (returns null) so the
    // standard POLOXI answer is shown instead — never a partially-scrubbed hybrid.
    private static WideLegalAnswerDto? ValidateLegalAnswer(LegalAnswerProposal? proposal,IReadOnlyList<WideExternalKnowledgeSnippet> verifiedAuthorities,string? winnerDisplayName,WideResolutionDeliverableDto? resolutionDeliverable,string interpretationStatus)
    {
        if(proposal is null)return null;
        if(string.IsNullOrWhiteSpace(proposal.Conclusion)||string.IsNullOrWhiteSpace(proposal.Analysis)||string.IsNullOrWhiteSpace(proposal.Application)||string.IsNullOrWhiteSpace(proposal.BottomLine))
            return null;
        // Authority whitelist: a cited authority name must be a substring-match (either direction) of a
        // verified authority title. If the composer cited anything unverified, discard the whole answer.
        var authorities=new List<WideLegalAuthorityDto>();
        foreach(var cited in proposal.KeyAuthorities??[])
        {
            if(string.IsNullOrWhiteSpace(cited.Name)||string.IsNullOrWhiteSpace(cited.Relevance))continue;
            var citedName=cited.Name.Trim();
            var matched=verifiedAuthorities.Any(authority=>
                authority.Title.Contains(citedName,StringComparison.OrdinalIgnoreCase)||
                citedName.Contains(authority.Title,StringComparison.OrdinalIgnoreCase));
            if(!matched)return null; // invented authority → discard entirely
            authorities.Add(new(citedName,cited.Relevance.Trim(),string.IsNullOrWhiteSpace(cited.Reference)?null:cited.Reference.Trim()));
        }
        // Conclusion-consistency guard: only when POLOXI genuinely reached NO interpretation (UNRESOLVED)
        // does a definitive conclusion contradict POLOXI. A BLOCKED determinacy driven purely by absent
        // VERIFIED EVIDENCE must NOT suppress a RESOLVED model-derived interpretation — that interpretation
        // is presented with an explicit unverified-evidence disclaimer instead of being refused.
        if(string.Equals(interpretationStatus,"UNRESOLVED",StringComparison.OrdinalIgnoreCase)
            &&resolutionDeliverable is {DeterminacyCode:"BLOCKED"}&&LooksDefinitive(proposal.Conclusion))
            return null;
        // Winner-consistency guard: when POLOXI named a winning interpretation, the composed conclusion
        // must reference it (it may not silently substitute a different answer).
        if(!string.IsNullOrWhiteSpace(winnerDisplayName)
            &&!proposal.Conclusion!.Contains(winnerDisplayName,StringComparison.OrdinalIgnoreCase)
            &&!(proposal.BottomLine!.Contains(winnerDisplayName,StringComparison.OrdinalIgnoreCase))
            &&!(proposal.Application!.Contains(winnerDisplayName,StringComparison.OrdinalIgnoreCase)))
            return null;
        return new(
            proposal.Conclusion!.Trim(),
            proposal.Analysis!.Trim(),
            authorities,
            proposal.Application!.Trim(),
            string.IsNullOrWhiteSpace(proposal.Counterargument)?null:proposal.Counterargument.Trim(),
            string.IsNullOrWhiteSpace(proposal.RemainingUncertainty)?null:proposal.RemainingUncertainty.Trim(),
            string.IsNullOrWhiteSpace(proposal.DecisiveMissingFact)?null:proposal.DecisiveMissingFact.Trim(),
            proposal.BottomLine!.Trim());
    }

    // Heuristic: does the conclusion assert a definite outcome? Used only by the BLOCKED-determinacy
    // guard to catch a composer overstating a resolution POLOXI did not reach.
    private static bool LooksDefinitive(string? conclusion)
    {
        if(string.IsNullOrWhiteSpace(conclusion))return false;
    ...
    private async Task<IReadOnlyCollection<WideExternalKnowledgeSnippet>> GatherExternalKnowledgeAsync(WideSearchRequest request,Guid executionId,IReadOnlyCollection<WideBranchRecord> interpretiveBranches,int retrievalConcurrency,IReadOnlyList<LegalAuthorityReference> proposedLegalAuthorities,CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // budgeting, and dedup behavior.
            //
            // DB stays the source of truth for the default context (GENERAL); we do NOT force LEGAL as
            // the default. Instead, this is a legal-research product, so when a non-LEGAL query cannot
            // actually ground (the general web provider is disabled or has no API key) but legal grounding
            // IS available, we fall back to the legal path rather than returning zero evidence. A query
            // explicitly run under LEGAL always uses the legal path.
            int maximumQueries,maximumSnippets,cacheHours;
            // Concept -> authority map (DB-backed) used only in the legal path when a branch names no
            // explicit citation, so concept-only doctrines can still resolve to verifiable citations.
            IReadOnlyCollection<WideLegalConceptAuthorityDto> legalConceptMap=[];
            // Retrieval delegate takes the authority kind so the legal path can route a specific
            // authority to the correct source; the web path ignores it.
            Func<string,LegalAuthorityKind,CancellationToken,Task<IReadOnlyCollection<WideExternalKnowledgeSnippet>>> retrieve;
            // Effective flag: whether this execution grounds through the legal sources. Starts from the
            // explicit context and may be promoted below when the web provider is unusable.
            var useLegalGrounding=isLegalContext;
            if(!isLegalContext)
            {
                // Non-LEGAL context: try the general web provider first. If it is disabled or missing an
                // API key, fall back to legal grounding (when enabled) instead of returning nothing.
                var configuration=await wideRepository.GetExternalGroundingConfigurationAsync(request.TenantId,cancellationToken);
                if(configuration.Enabled&&!string.IsNullOrWhiteSpace(configuration.ApiKey))
                {
                    maximumQueries=configuration.MaximumQueriesPerExecution;
                    maximumSnippets=configuration.MaximumSnippetsPerQuery;
                    cacheHours=configuration.CacheHours;
                    retrieve=(query,_,token)=>externalKnowledgeProvider.SearchAsync(query,configuration,token);
                    goto sourceResolved;
                }
                // Web provider unusable: attempt legal fallback so the run still produces grounded evidence.
                var legalFallback=await wideRepository.GetLegalGroundingConfigurationAsync(request.TenantId,cancellationToken);
                if(!legalFallback.Enabled)
                {
                    logger.LogWarning("LEGAL-TRACE stage=0-router outcome=NO_SOURCE contextCode={ContextCode}; general web provider unusable (enabled={WebEnabled}, apiKeyPresent={ApiKeyPresent}) and legal grounding disabled. Zero grounding evidence will be admitted.",request.ContextCode,configuration.Enabled,!string.IsNullOrWhiteSpace(configuration.ApiKey));
                    return [];
                }
                logger.LogInformation("LEGAL-TRACE stage=0-router outcome=FALLBACK_LEGAL contextCode={ContextCode}; general web provider unusable (enabled={WebEnabled}, apiKeyPresent={ApiKeyPresent}), routing through legal grounding sources instead.",request.ContextCode,configuration.Enabled,!string.IsNullOrWhiteSpace(configuration.ApiKey));
                useLegalGrounding=true;
            }
            {
                var legalConfiguration=await wideRepository.GetLegalGroundingConfigurationAsync(request.TenantId,cancellationToken);
                if(!legalConfiguration.Enabled)return [];
                maximumQueries=legalConfiguration.MaximumQueriesPerExecution;
                maximumSnippets=legalConfiguration.MaximumSnippetsPerQuery;
                cacheHours=legalConfiguration.CacheHours;
                retrieve=(query,kind,token)=>legalRetriever.SearchAsync(query,legalConfiguration,kind,token);
                legalConceptMap=await wideRepository.GetLegalConceptAuthoritiesAsync(request.TenantId,cancellationToken);
                // Loud, explicit signal when the DB-backed concept->authority map is empty in a legal
                // grounding run: concept-only doctrines (cure, waiver, perfect tender, ...) then resolve
                // to NO citation, the citation-driven retrievers never fire, and grounding silently
                // returns zero evidence. An empty map almost always means migration 0184 has not been
                // applied to this database, so surface it as a warning instead of failing quietly.
                if(legalConceptMap.Count==0)
                    logger.LogWarning("LEGAL-TRACE stage=0-conceptmap outcome=EMPTY tenantId={TenantId}; POLOXI.Legal_LegalConceptAuthority returned 0 rows. Concept-only legal questions cannot resolve to a citation and will admit zero grounding evidence. Verify migration 0184_IntelligenceWideLegalConceptAuthority.sql has been applied to this database.",request.TenantId);
            }
            sourceResolved:
            var notBeforeUtc=DateTime.UtcNow.AddHours(-cacheHours);
            var targets=interpretiveBranches.OrderBy(item=>item.LevelNumber).ThenByDescending(item=>item.Confidence).Take(maximumQueries).ToArray();
            ...
            // A specific authority is searched with a CLEAN provider query (the authority name only) so
            // relevance is not diluted, then the retrieved snippets are stamped with explicit branch
            // attribution (BranchId/BranchDisplayName) so ComputeEvidenceSupport links them to the branch
            // deterministically. The attribution query (containing the branch display name) is retained
            // for the cache key and legacy fallback. Verification drops snippets that do not actually
            // reference the authority. When no authority is supplied (concept fallback / web),
            // providerQuery == attributionQuery.
            async Task<IReadOnlyList<WideExternalKnowledgeSnippet>> RetrieveStampedAsync(WideBranchRecord branch,string providerQuery,string attributionQuery,LegalAuthorityKind kind,LegalAuthorityReference? verify)
            {
                ...
                if(verify is{}authority)
                {
                    ...
                    // dropped is distinguishable from one merely low on proposition support.
                    var identityMatched=classified.Count(item=>item.classification.IdentityVerified);
                    logger.LogInformation("LEGAL-TRACE stage=5-identity branchId={BranchId} authority=\"{Authority}\" retrieved={Retrieved} identityMatched={Matched} details=[{Details}]",branch.WideBranchId,authority.Query,retrieved.Count,identityMatched,string.Join(" | ",classified.Take(5).Select(item=>$"\"{Truncate(item.snippet.Title,80)}\" identity={item.classification.IdentityVerified} support={item.classification.SupportScore:F2} status={item.classification.Status}")));
                    stamped=classified
                        .Where(item=>item.classification.IdentityVerified)
                        .Take(maximumSnippets)
                        .Select(item=>StampBranch(item.snippet,branch,attributionQuery) with
                        {
                            AuthorityKind=authority.Kind.ToString(),
                            AuthorityIdentityVerified=true,
                            PropositionSupportScore=item.classification.SupportScore,
                            PropositionSupportStatus=item.classification.Status
                        })
                        .ToArray();
                    // STAGE 7 (branch attribution): confirm verified snippets carry the explicit BranchId
                    // used by ComputeEvidenceSupport (not a fragile query-substring match).
                    if(stamped.Count>0)
                        logger.LogInformation("LEGAL-TRACE stage=7-attribution branchId={BranchId} authority=\"{Authority}\" verifiedSnippets={Count} allCarryBranchId={AllCarry}",branch.WideBranchId,authority.Query,stamped.Count,stamped.All(snippet=>snippet.BranchId==branch.WideBranchId));
                }
                else
                {
                ...
                // STAGE 8 (persistence): confirm verified snippets were persisted under the attribution key.
                logger.LogInformation("LEGAL-TRACE stage=8-persist branchId={BranchId} attributionQuery=\"{AttributionQuery}\" persisted={Count}",branch.WideBranchId,attributionQuery,stamped.Count);
                return stamped;
            }
            await Task.WhenAll(targets.Select(async(branch,index)=>
            {
                ...
                try
                {
                    ...
                    // LEGAL: search the specific authorities the branch cites, routed by type and verified.
                    // Fall back to the concept query when the branch names no recognizable authority.
                    ...
                    {
                        // STAGE 1 (authority extraction): show whether a named authority was recognized in
                        // the branch text and, if not, that we fall back to a targeted DOCTRINAL query
                        // (decisive doctrinal terms routed to both legal sources) rather than the diluted
                        // whole-question concept query.
                        if(authorities.Count>0)
                            logger.LogInformation("LEGAL-TRACE stage=1-extraction branchId={BranchId} branch=\"{Branch}\" extracted={Count} authorities=[{Authorities}]",branch.WideBranchId,branch.DisplayName,authorities.Count,string.Join(" | ",authorities.Select(a=>$"{a.Query} (kind={a.Kind}, tokens=[{string.Join(",",a.VerificationTokens)}])")));
                        else
                        {
                            var doctrinalQuery=BuildDoctrinalFallbackQuery(request.Query,branch);
                            logger.LogInformation("LEGAL-TRACE stage=1-extraction branchId={BranchId} branch=\"{Branch}\" extracted=0 fallback=DOCTRINAL doctrinalQuery=\"{DoctrinalQuery}\" conceptQuery=\"{ConceptQuery}\"",branch.WideBranchId,branch.DisplayName,doctrinalQuery,conceptQuery);
                        }
                    }
                    if(authorities.Count>0)
                    {
                    ...
                    else if(useLegalGrounding)
                    {
                        // Defect C: extraction=0 in the LEGAL context now routes the decisive doctrinal terms
                        // into targeted CourtListener/GovInfo searches (kind=Any queries both sources). If the
                        // doctrinal query yields nothing, fall back once more to the generic concept query so
                        // behavior never regresses below the previous fallback.
                        var doctrinalQuery=BuildDoctrinalFallbackQuery(request.Query,branch);
                        var doctrinalSnippets=await RetrieveStampedAsync(branch,doctrinalQuery,doctrinalQuery,LegalAuthorityKind.Any,null);
                        results[index]=doctrinalSnippets.Count>0
                            ?doctrinalSnippets
                            :await RetrieveStampedAsync(branch,conceptQuery,conceptQuery,LegalAuthorityKind.Any,null);
                    }
                    else
                    {
            ...
            if(useLegalGrounding)
            {
                ...
                var branchCount=targets.Length;
                var totalSnippets=deduped.Length;
                var verifiedAuthorities=deduped.Count(snippet=>snippet.AuthorityIdentityVerified);
                var withBranchId=deduped.Count(snippet=>snippet.BranchId is not null);
                logger.LogInformation("LEGAL-TRACE stage=10-final branches={Branches} totalSnippets={Total} verifiedAuthorities={Verified} withBranchId={WithBranchId}",branchCount,totalSnippets,verifiedAuthorities,withBranchId);
            }
            return deduped;
        }
        catch(Exception)when(!cancellationToken.IsCancellationRequested)
    ...
    private static WideResolutionDeliverableDto? BuildResolutionDeliverable(WideSearchRequest request,WideConfiguration configuration,WideQueryContract? queryContract,IReadOnlyCollection<WideAmbiguityGroupDto> ambiguityGroups,PoloxiEvidenceDto[] relevantEvidence,IReadOnlyCollection<WideExternalKnowledgeSnippet> externalKnowledge,decimal? decisionConfidence,decimal decisionEvidenceCoverage,WideEntropyResult finalEntropy,WideCandidateDto[] topCandidates,IReadOnlyCollection<WideInterpretiveResultDto> interpretiveResults)
    {
        ...
        // Defect D (extended): "retrieved" is not the same as "admitted". A run can RETRIEVE external
        // authorities that the evidence-admission gate then rejects (e.g. legal snippets that name but do
        // not support the branch proposition), leaving decision coverage and confidence at 0 even though
        // hasEvidence is true. Admission is measured by what actually reached the decision, never by raw
        // retrieval counts, so the admission gate is preserved - not lowered.
        var hasAdmittedEvidence=coverage>0m||confidence>0m;
        ...
        // Defect D: a run can produce strong interpretive analysis (doctrine, reasoning, ranked items)
        // even when no external/enterprise evidence was admitted. Detect that so the deliverable can
        // preserve and surface the reasoning instead of collapsing to a blocked/null outcome.
        var groundedInterpretations=(interpretiveResults??[]).Where(result=>result is not null&&!string.IsNullOrWhiteSpace(result.Interpretation)).ToArray();
        var hasInterpretiveAnalysis=groundedInterpretations.Length>0;
        ...
        // When no evidence was ADMITTED but interpretive analysis exists, qualify (do not block): the
        // analysis stands on model reasoning and must be flagged as unverified rather than discarded.
        if(!hasAdmittedEvidence&&!hasInterpretiveAnalysis)blockingInputs.Add("No grounded facts or rules were retrieved for the supplied inputs; the outcome cannot be computed from the material provided.");
        else if(!hasAdmittedEvidence&&hasInterpretiveAnalysis)blockingInputs.Add(hasEvidence?"The analysis below is derived from model reasoning; the retrieved sources were not admitted as supporting evidence, so verify against authoritative sources before relying on it.":"The analysis below is derived from model reasoning without admitted external or enterprise evidence; verify against authoritative sources before relying on it.");
        ...
        // Output/presentation requirements (headings, brevity, structure) are NOT deciding facts, so they
        // must never be reported as "missing grounding" when the interpretive analysis already delivers the
        // substantive content. Only surface them as blocking when the analysis itself is absent.
        if(outputRequirements.Length>0&&coverage<0.5m&&!hasInterpretiveAnalysis)
        ...
        // Determinacy: RESOLVED needs admitted evidence, adequate coverage, and low residual uncertainty.
        // UNVERIFIED preserves interpretive analysis produced without ADMITTED evidence (defect D): the
        // reasoning is delivered, but the run is explicitly marked as not evidence-grounded. This covers
        // both "nothing retrieved" and "retrieved but rejected by the admission gate" so a fully-analyzed
        // run is never demoted to BLOCKED - the deliverable is never inferior to the interpretive analysis.
        string determinacy;
        if(hasAdmittedEvidence&&coverage>=0.5m&&uncertainty<=0.5m&&blockingInputs.Count==0)determinacy="RESOLVED";
        else if(hasAdmittedEvidence)determinacy="PARTIAL";
        else if(hasInterpretiveAnalysis)determinacy="UNVERIFIED";
        else determinacy="BLOCKED";
        var citations=BuildResolutionCitations(relevantEvidence,externalKnowledge);
        var reason=BuildResolutionReason(determinacy,target,relevantEvidence,externalKnowledge,coverage,confidence,uncertainty,groundedInterpretations);
        var headline=determinacy switch
        {
            ...
            "UNVERIFIED"=>$"POLOXI analyzed {target} from reasoning, but the conclusion is unverified — no external or enterprise evidence was admitted.",
            _=>$"POLOXI cannot resolve {target} yet — required inputs are missing."
        };
        // Defect D: for UNVERIFIED runs, carry the strongest interpretive analysis forward as the outcome
        // so the substantive reasoning survives instead of the outcome collapsing to null.
        var outcome=determinacy switch
        {
            "BLOCKED"=>null,
            "UNVERIFIED"=>BuildResolutionOutcome(target,relevantEvidence,externalKnowledge,topCandidates)??BuildInterpretiveOutcome(groundedInterpretations),
            _=>BuildResolutionOutcome(target,relevantEvidence,externalKnowledge,topCandidates)
        };
        return new(determinacy,headline,outcome,reason,blockingInputs,citations,confidence,coverage,uncertainty);
    }

    // Defect D: compose an outcome string from the strongest interpretive result when no evidence-backed
    // outcome exists, so reasoning-only runs still deliver their substantive analysis.
    private static string? BuildInterpretiveOutcome(WideInterpretiveResultDto[] interpretiveResults)
    {
        var strongest=interpretiveResults.OrderByDescending(result=>result.Confidence).FirstOrDefault();
        if(strongest is null)return null;
        var label=string.IsNullOrWhiteSpace(strongest.BranchDisplayName)?strongest.Interpretation.Trim():strongest.BranchDisplayName.Trim();
        var detail=TrimDescription(strongest.Interpretation);
        return string.IsNullOrWhiteSpace(detail)||string.Equals(label,detail,StringComparison.OrdinalIgnoreCase)?label:$"{label} — {detail}";
    }
    ...
    private static string BuildResolutionReason(string determinacy,string target,PoloxiEvidenceDto[] relevantEvidence,IReadOnlyCollection<WideExternalKnowledgeSnippet> externalKnowledge,decimal coverage,decimal confidence,decimal uncertainty,WideInterpretiveResultDto[] interpretiveResults)
    {
        var builder=new StringBuilder();
        ...
        if(determinacy=="UNVERIFIED")
        {
            var strongestInterpretation=interpretiveResults.OrderByDescending(result=>result.Confidence).FirstOrDefault();
            builder.Append("Reasoned analysis for ").Append(target).Append(": ");
            if(strongestInterpretation is not null&&!string.IsNullOrWhiteSpace(strongestInterpretation.BranchDisplayName))builder.Append("led by the \"").Append(strongestInterpretation.BranchDisplayName.Trim()).Append("\" interpretation");
            else builder.Append("derived from the interpretive analysis");
            if(externalCount>0||enterpriseCount>0)
                builder.Append(". POLOXI retrieved ").Append(enterpriseCount).Append(" enterprise and ").Append(externalCount)
                    .Append(" external source(s), but none were admitted as supporting evidence for the deciding propositions, so this conclusion rests on model reasoning at ");
            else
                builder.Append(". No external or enterprise evidence was admitted (0 grounded items), so this conclusion rests on model reasoning at ");
            builder.Append(confidence.ToString("P0")).Append(" decision confidence with ").Append(uncertainty.ToString("P0"))
                .Append(" residual uncertainty. Treat it as unverified and confirm against authoritative sources before relying on it.");
            return builder.ToString();
        }
...
""";
    ...
    private async Task<IReadOnlyCollection<WideExternalKnowledgeSnippet>> GatherSeedVerificationKnowledgeAsync(WideSearchRequest request,Guid executionId,IReadOnlyCollection<string> seeds,CancellationToken cancellationToken)
    {
        ...
        {
            var isLegalContext=string.Equals(request.ContextCode?.Trim(),WideSearchContexts.Legal,StringComparison.OrdinalIgnoreCase);
            // Candidate verification grounding must use the SAME context source as branch grounding:
            // LEGAL routes through the legal retrievers, everything else uses the general web provider.
            int maximumSnippets,cacheHours;
            Func<string,CancellationToken,Task<IReadOnlyCollection<WideExternalKnowledgeSnippet>>> retrieve;
            if(isLegalContext)
            {
                var legalConfiguration=await wideRepository.GetLegalGroundingConfigurationAsync(request.TenantId,cancellationToken);
                if(!legalConfiguration.Enabled)return [];
                maximumSnippets=legalConfiguration.MaximumSnippetsPerQuery;
                cacheHours=legalConfiguration.CacheHours;
                retrieve=(query,token)=>legalRetriever.SearchAsync(query,legalConfiguration,LegalAuthorityKind.Any,token);
            }
            else
            {
                var configuration=await wideRepository.GetExternalGroundingConfigurationAsync(request.TenantId,cancellationToken);
    ...
    private static decimal ComputeEvidenceSupport(WideBranchRecord branch,IReadOnlyCollection<PoloxiEvidenceDto> evidence,IReadOnlyCollection<WideExternalKnowledgeSnippet> externalKnowledge,WideConfiguration configuration)
    {
        ...
        // External snippets attribute to a branch via explicit BranchId (stable, deterministic). Legacy
