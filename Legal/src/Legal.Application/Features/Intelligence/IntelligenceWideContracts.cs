...
// Database-backed context option for the wide-search Context dropdown (POLOXI.Legal_SearchContext).
public sealed record WideSearchContextDto(string ContextCode,string DisplayName,string? Description,bool IsDefault);

// Well-known search context codes (POLOXI.Legal_SearchContext.ContextCode).
public static class WideSearchContexts
{
    public const string General="GENERAL";
    public const string Legal="LEGAL";
}

// A single Legal Grounding configuration row (Core.ConfigurationSetting) surfaced on /legal/configuration.
// IsEncrypted values (CourtListener token, GovInfo key) are masked for display; SettingValue carries the
// masked placeholder on read and the new plain value on save.
public sealed record LegalGroundingSettingDto(
    string SettingKey,
    string SettingValue,
    string DataTypeCode,
    string? Description,
...
// Create/update request for a Legal Grounding configuration row (Platform scope, ModuleCode=Intelligence).
public sealed record SaveLegalGroundingSettingRequest(
    string SettingKey,
    string SettingValue,
    string DataTypeCode,
    string? Description,
...
public sealed record WideSearchResponse(Guid WideExecutionId,string Query,string StatusCode,string TerminationReasonCode,int DepthReached,int LlmCallCount,decimal FinalConfidence,string AnswerVerificationCode,string? FinalAnswer,IReadOnlyCollection<WideBranchDto> Branches,IReadOnlyCollection<PoloxiEvidenceDto> Evidence,IReadOnlyCollection<WideActionSuggestionDto> SuggestedActions,long DurationMilliseconds)
{
    ...
    // Legal Answer Composer output (presentation ONLY, legal context + POLOXI engine). The senior-
    // attorney-style rendering of the decision POLOXI already made. Null for non-legal context, LLM-only
...
// Legal Answer Composer (presentation ONLY). This is the senior-attorney-style rendering of the
// decision POLOXI already made — it introduces NO new reasoning, NO new authorities, and NEVER
// changes the POLOXI conclusion. It is produced by one fast LLM call AFTER convergence and gated by a
// deterministic output guard: if the composer cites an authority POLOXI did not verify or contradicts
// the POLOXI conclusion, the whole composed answer is discarded and the standard POLOXI answer stands.
// Null whenever the composer did not run (non-legal context, LLM-only mode) or failed the guard.
public sealed record WideLegalAnswerDto(
    string Conclusion,
    string Analysis,
    IReadOnlyCollection<WideLegalAuthorityDto> KeyAuthorities,
    string Application,
    string? Counterargument,
    string? RemainingUncertainty,
    // The single missing fact most likely to reverse the conclusion — POLOXI's ambiguity discipline
    // surfaced in attorney terms ("this is the leading conclusion, and this is what could change it").
    string? DecisiveMissingFact,
    string BottomLine);

// A single authority cited by the Legal Answer Composer. Name/Reference must match an authority POLOXI
// already verified (present in the verified external-knowledge set); Relevance is the plain-language
...
// Legal-context grounding configuration loaded from Core.ConfigurationSetting (DB is the source of truth).
// Selected when the request ContextCode is LEGAL: retrieval is served by CourtListener (case law) and
// GovInfo/eCFR (statutes/regulations). A disabled flag or blank key/token disables that individual source.
public sealed record WideLegalGroundingConfiguration(
    bool Enabled,
    int MaximumQueriesPerExecution,
    int MaximumSnippetsPerQuery,
    int CacheHours,
    int TimeoutSeconds,
    bool CourtListenerEnabled,
...
// fetch and the mandatory identity gate can verify. ConceptKeywords is a space-delimited AND-match;
// VerificationTokens is a comma-delimited list of distinctive tokens a retrieved source must contain.
public sealed record WideLegalConceptAuthorityDto(
...
// convenience constructor. Dapper requires a parameterless constructor to materialize rows via property
// setters; a positional record exposes only its primary constructor, so Dapper could not populate the
// extra settable columns (BranchId, AuthorityKind, etc.) and threw 'a parameterless default constructor
// ... is required'. The 6-arg constructor preserves the existing positional call sites in the legal
// source adapters, and all core properties are settable so Dapper can hydrate every column.
public sealed record WideExternalKnowledgeSnippet
{
    public WideExternalKnowledgeSnippet(){}

    public WideExternalKnowledgeSnippet(string query,string title,string url,string snippet,decimal score,DateTime retrievedDateUtc)
    {
        Query=query;
        Title=title;
        Url=url;
        Snippet=snippet;
        Score=score;
        RetrievedDateUtc=retrievedDateUtc;
    }
    ...
    public DateTime RetrievedDateUtc{get;set;}

    // Explicit, stable branch attribution: which branch this snippet was retrieved for. Preferred over
    // the legacy query-substring match (snippet.Query.Contains(branch.DisplayName)) because branch names
    // can change, overlap, or accidentally match unrelated query text. Null for general-web snippets and
    // legacy cache rows, which fall back to the query-based attribution.
    public Guid? BranchId{get;set;}
    public string? BranchDisplayName{get;set;}

    // Legal evidence-verification signals (deterministic, no LLM). Null/default for general-web snippets
    ...
    // PropositionSupportScore (0..1): deterministic overlap between the branch claim and the snippet text.
    // A weighting signal for evidence contribution, NOT a declaration of legal truth.
    public decimal PropositionSupportScore{get;set;}
    // PropositionSupportStatus: VERIFIED_SUPPORT | AUTHORITY_FOUND_BUT_SUPPORT_UNCLEAR | UNVERIFIED.
    public string? PropositionSupportStatus{get;set;}
}
